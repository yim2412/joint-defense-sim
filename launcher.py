"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v10.16 — PyQt6 런처                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v10.16 — v11.x 계획 수립: 검증·UI정리·성능·시나리오 5개 항목]             ║
║  UPD-A  _PLANS: v11.1~v11.5 추가 (검증·계획·UI정리·성능·시나리오라이브러리)  ║
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v11.2", "낮음", "v11.x 새 계획 수립",
             ...
             "후보 항목: 지도 오버레이(Folium/MapLibre) · 멀티 편대 동시 시뮬 · "
             "실시간 대시보드(WebSocket) · 시나리오 저장/공유 포맷 표준화 · "
             "AI 운용 자문 모드(GPT API 연동). 우선순위 합의 후 확정."),
            ("v11.3", "중간", "설정 패널 UI 전면 정리",
             "기능 추가로 복잡해진 설정 패널 재구성. "
             "항목 그루핑 재설계: 기본/환경/방어전술/항공자산/고급 5탭으로 분리. "
             "체크박스 설명 툴팁 통일 · 기본값 표시 · 프리셋 빠른 적용 버튼. "
             "기존 기능 변경 없이 UI 레이아웃만 개선."),
            ("v11.4", "중간", "MC 성능 최적화",
             "MC 10,000회 이상 고속 실행 목표. "
             "현재 병렬 배치 처리 외 추가 최적화: "
             "① SimV7 핫패스 numpy 벡터화 (위치 갱신 루프) "
             "② 불필요한 SimFrame 기록 MC 모드에서 비활성화 "
             "③ LHS 샘플링 기본값 확대 (현재 sobol_npp=3 → 5). "
             "phase_times 프로파일러로 병목 측정 후 상위 2개 구간 집중 개선."),
            ("v11.5", "낮음", "시나리오 라이브러리",
             "실제 교리 기반 전술 시나리오 프리셋 추가. "
             "① 서해 차단작전: 중국 상륙함대 vs 한국 기동전단 + 공군 CAP "
             "② 독도 방어: 다방위 동시 포화공격 + 해협 협착 기동 "
             "③ 동해 항모전단 요격: KF-21·KSS-III 협동 타격 "
             "④ 북한 포화도발 대응: 탄도/순항 혼합 대량 발사 시나리오. "
             "각 시나리오: cfg JSON + 설명 텍스트 + 권장 편성 포함."),
        ]
