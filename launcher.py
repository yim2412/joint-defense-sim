"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v10.20 — PyQt6 런처                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v10.20 — v18~v20 육해공 통합 합동작전 로드맵 추가]                        ║
║  UPD-A  _PLANS v18.1~5: 공군 작전급 (제공권·SEAD·전략폭격)                  ║
║  UPD-B  _PLANS v19.1~4: 육군 작전급 (지상군·방공·상륙)                      ║
║  UPD-C  _PLANS v20.1~4: 육해공 통합 합동작전 (JCS·Joint Fires·시나리오)     ║
...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            # ── v11.x — 기반 정비 ─────────────────────────────────────────────
            ("v11.1", "낮음", "검증 시뮬 실행",
             "수정된 수치(SM-3 650km·YJ-18 Mach3.5·PHY-1~12)가 실제 시뮬에서 예상대로 동작하는지 확인. "
             "기준 시나리오(기동전단 기본 vs 랴오닝 항모전단) MC 500회 → 요격률·피격·비용 변화량 분석."),
            ("v11.2", "낮음", "차기 계획 수립",
             "v12~v16 항목 우선순위 최종 합의. 구현 순서·의존성 정리."),
            ("v11.3", "중간", "설정 패널 UI 전면 정리",
             "기본/환경/방어전술/항공자산/고급 5탭으로 분리. "
             "체크박스 툴팁 통일 · 기본값 표시 · 프리셋 빠른 적용 버튼. 기존 기능 변경 없음."),
            ("v11.4", "중간", "MC 성능 최적화",
             "MC 10,000회 이상 고속 실행 목표. "
             "① SimV7 핫패스 numpy 벡터화 ② SimFrame 기록 MC 모드 비활성화 "
             "③ LHS sobol_npp=3→5. phase_times 병목 측정 후 상위 구간 집중."),
            ("v11.5", "낮음", "시나리오 라이브러리",
            ...
            ("v12.1", "매우 높음", "실제 유도 알고리즘 (PNG)",
             ...
             "MissileObj에 heading·normal_accel 추가. 요격 가능성 물리 기반 계산."),
            ("v12.2", "매우 높음", "수중 음향 전파 모델",
             "단순 소나 탐지거리 → SONAR 방정식(TL·NL·DT·SL) 완전 구현. "
             "WOA18 수온으로 실제 음파 굴절 경로 계산. "
             "수렴 지대(CZ)·음영 구역·직접 경로 구분. 잠수함 탐지가 물리적으로 결정."),
            ...
            ("v12.4", "중간", "동적 기상 변화",
             ...
             "기상청 계절별 패턴 기반 확률적 날씨 전이. 탐지·교전 능력 실시간 변동."),
            ("v12.5", "중간", "피아식별 오류 (IFF)",
             "IFF 실패 확률 모델. 식별 지연 + 오인식 → 아군 오사 가능성. "
             "혼잡 전장에서 ROE 이행 지연. 아군기 오분류 상황 재현."),
            # ── v13.x — 시각화 & 인터페이스 ──────────────────────────────────
            ...
            ("v13.2", "중간", "C4ISR 네트워크 가시화",
             ...
             "어느 함정 먼저 격침 시 방어망 붕괴하는지 네트워크 취약점 분석."),
            ("v13.3", "중간", "실시간 다중 KPI 대시보드",
             "시뮬 중단마다 15+ 지표 실시간 계산: "
             "탄약 소모 효율·비용당 격침 비율·피아 교환비·방어 포화도. "
             "전술 의사결정 모드(v10.7)와 시너지 — 무기 교체 판단 즉시 제공."),
            ("v13.4", "중간", "위성 정찰 재방문 주기",
             "궤도 SAR/EO 위성 재방문 주기(12~24h) 시뮬. "
             "창 열릴 때만 OTH 목표 지정 가능. 공백 시간 → 아군 기습 기회 창."),
            # ── v14.x — AI & 자율화 ────────────────────────────────────────────
            ("v14.1", "매우 높음", "적응형 전술 AI (MCTS)",
             "Monte Carlo Tree Search 기반 적 지휘관 AI. "
             "교전 구간마다 아군 편성·피해 분석 후 전술 실시간 전환: "
             "포화공격→분산접근→기만 적응. 난이도 슬라이더로 AI 탐색 깊이 조정."),
            ("v14.2", "높음", "ML 기반 즉시 예측",
             "MC 결과로 학습한 신경망(sklearn/PyTorch). "
             "설정 바꾸는 즉시 요격률·피해 실시간 업데이트 (MC 없이). "
             "모델 재학습 버튼 → 새 DB 수치 반영."),
            ("v14.3", "높음", "함정별 자율 AI (Battle Management)",
             "각 함정이 독립 AI 에이전트. CEC·지령 없이도 자율 탐지·사격 판정. "
             "기함 격침 시 차순위 함정이 자동 지휘권 인수. 자율화 수준 슬라이더."),
            ("v14.4", "매우 높음", "무인기 군집 (Swarm)",
             "UAV·USV·UUV 수십~수백 드론 분산 경로 동시 접근. "
             "개별 요격 필요 → 탄약 급소모. Boids 알고리즘 기반 군집 AI. "
             "아군 ISR 드론 배치 → 탐지 범위 확장 전술."),
            ("v14.5", "매우 높음", "ISR 드론 군집 (정찰)",
             "고정익 무인정찰기(RQ-4 유사) 편대 배치. 탐지 범위 확장 + 미사일 OTH 유도 정보 제공. "
             "드론 개별 피격·격추. 배치 비용 vs 탐지 이득 트레이드오프."),
            # ── v15.x — 전장 도메인 확장 ──────────────────────────────────────
            ("v15.1", "높음", "전자전 고도화 + ESM 역탐지",
             "재밍 포드·DRFM 기만기·SIGINT. 주파수 대역별 세부 ECM 모델. "
             "적 ESM: 아군 레이더 신호 → 방향·세기 파악 → 역방향 미사일 발사. "
             "레이더 ON/OFF 타이밍이 전술 핵심 결정점으로 격상."),
            ("v15.2", "높음", "다전장 연동 (공군·지상)",
             "공군 SEAD·지상 패트리엇/THAAD 동시 운용. "
             "합동 교전권(ROE) 계층 — 공군 담당 구역에 함정 SAM 발사 금지. "
             "지상·공중·해상 통합 방공 구역 시각화."),
            ("v15.3", "높음", "사이버전 모듈",
             "적 사이버 공격: CEC 패킷 변조 → 함정 오발사. CIC 부분 마비 → 탐지 지연 급증. "
             "아군 반격: 적 SAM 레이더 교란. 사이버전 강도 슬라이더 + 패치 시간 변수."),
            ("v15.4", "높음", "분산 해양작전 (DMO)",
             "미 해군 Distributed Lethality 교리. "
             "소형 함정 분산 배치 + 개별 타격 → 집중 방어 회피. "
             "PKG·PCC 군집이 독립 공격망 운용."),
            ("v15.5", "높음", "해안 방어 시설 통합 (C-RAM)",
             "육상 C-RAM·해안 미사일 포대 고정 배치. "
             "기뢰 차단 구역 설정. 인천·부산·동해 항만 방어 시나리오. "
             "한반도 지형(v13.1)과 연동 권장."),
            ("v15.6", "중간", "ECM 좌표 기만",
             "함정 ECM 강화 시 레이더 화면상 표시 위치 ≠ 실제 위치. "
             "적 미사일이 기만 좌표로 유도 → 명중률 저하. "
             "현재 탐지거리 감소 ECM(v10.x)과 별개 레이어."),
            # ── v16.x — 군수·미래 전장 ────────────────────────────────────────
            ("v16.1", "높음", "군수·보급 모델",
             "보급함(AOE) 이동·체류 시간 모델링. VLS 고갈 후 귀환·재장전 시간 시뮬. "
             "연료 소모 → 고속 기동 시 작전 반경 축소. 장시간 작전 전술 판단점 추가."),
            ("v16.2", "높음", "지향성 에너지 무기 (레이저·HPM)",
             "HELIOS 60kW 레이저 + 고출력 마이크로파. "
             "무한 탄약이지만 함정 발전기 전력 제한. "
             "Swarm·UAV(v14.4/5)에 특히 효과적. 현무-4 초음속은 레이저 무력화."),
            ("v16.3", "높음", "멀티플레이어 대전",
             "WebSocket 서버. 한 명 이지스 함대, 다른 한 명 적 항모전단 실시간 대전. "
             "관전 모드 + 대전 기록 시스템."),
            # ── v17.x — 작전급 시뮬레이터 ─────────────────────────────────────
            # 선행 필수: v11.4(MC성능) · v14.2(ML예측) · v16.1(군수보급)
            # 캠페인 엔진이 현재 SimV7을 내부 교전 해결기로 재사용
            ("v17.1", "매우 높음", "캠페인 엔진 기반 설계",
             "이벤트 기반(Event-Driven) 캠페인 아키텍처 설계·구현. "
             "CampaignEngine: ForcePool(전력 상태) + MissionQueue(임무 큐) + EventHeap(우선순위 이벤트). "
             "1틱 = 1시간. 72시간 캠페인이 수초 내 완료되는 것이 목표. "
             "현재 SimV7을 EngagementResolver로 내장 — 교전 발생 시 호출."),
            ("v17.2", "높음", "전력 관리 모델 (ForcePool)",
             "전 함정 실시간 상태 추적: 출항 / 초계 중 / 귀항 / 수리 대기 / 수리 중 / 재편성. "
             "손상 함정 수리 기간(피해 심각도별 1~14일), 탄약 재보급 시간. "
             "작전 가용 전력 = 전체 전력 − 수리 중 − 귀항 중. 실시간 전력 변화 추적."),
            ("v17.3", "높음", "임무 배정 시스템 (MissionAssigner)",
             "가용 전력을 복수 임무에 자동 배정: 초계·호위·타격·대잠·보급 호위. "
             "우선순위 기반 배정 알고리즘 (이지스급 → 고위협 구역 우선). "
             "임무 충돌 해결: 같은 함정에 두 임무가 동시 요구될 때 처리."),
            ("v17.4", "높음", "정보 불확실성 모델 (Intel)",
             "적 전력 위치를 '확실'이 아닌 확률 분포로 관리. "
             "탐지 자산(위성·P-8A·잠수함) 재방문 주기마다 정보 갱신. "
             "정보 신선도: 오래된 정보일수록 위치 불확실성 반경 확대. "
             "불확실 정보로 임무 배정 시 위험도 계산."),
            ("v17.5", "높음", "해상 교통로 통제 모델 (SLOC)",
             "한반도 주요 SLOC 3개(서해·대한해협·동해) 통제 상태 시뮬. "
             "적 함정·잠수함이 특정 해역에 진입하면 SLOC 위협도 상승. "
             "SLOC 차단 지속 시간 → 보급 감소 → 전력 작전 지속 능력 하락. "
             "아군 초계 강도에 따른 SLOC 안전도 변화."),
            ("v17.6", "중간", "캠페인 MC 분석",
             "캠페인 전체를 N회 반복 → 전역 결과 통계. "
             "출력: 72시간 후 아군 잔존 전력 분포 / SLOC 통제 성공률 / 전역 비용. "
             "변수 민감도 분석: 어떤 초기 조건이 전역 결과에 가장 큰 영향? "
             "v14.2 ML 예측으로 개별 교전 해결 → 캠페인 MC 1,000회가 수분 내 완료."),
            ("v17.7", "중간", "캠페인 보고서 & 시각화",
             "전역 타임라인: 시각별 전력 상태·임무 현황·SLOC 통제 변화 그래프. "
             "결정적 순간(전력 30% 이하, SLOC 차단) 자동 표시. "
             "v13.1 3D 지도에 캠페인 경로 오버레이. "
             "Excel/PDF 전역 보고서 자동 생성."),
            # ── v18.x — 공군 작전급 ────────────────────────────────────────────
            # 선행 필수: v17 해군 작전급 완성 · v15.2 다전장 연동
            ("v18.1", "매우 높음", "공군 전력 DB & 임무 모델",
             "한국·미국 공군 전력 DB 구축: KF-21·F-35A·F-15K·F-16·B-1B·B-52 등. "
             "공군 임무 유형: CAP(제공권)·CAS(근접항공지원)·SEAD/DEAD·전략폭격·ISR. "
             "출격 준비시간·항속거리·탑재량·기지별 배치 현황 모델링."),
            ("v18.2", "높음", "제공권 통제 모델",
             "공중 우세 구역(Air Superiority Zone) 시뮬. "
             "아군 CAP 밀도·적 방공망 강도에 따른 제공권 통제 확률 계산. "
             "제공권 상실 시 해군 작전 패널티 연동 (v17 캠페인 엔진). "
             "1시간 단위 제공권 지도 업데이트."),
            ("v18.3", "높음", "SEAD/DEAD 작전 모델",
             "적 방공망(SA-21·HHQ-9·S-400) 탐지·타격 캠페인. "
             "SEAD 성공 시 해군 작전 구역 확대, 실패 시 해군 SM-3 수요 증가. "
             "방공망 복구 시간 모델: 타격 후 6~48시간 재가동. "
             "해군 전자전(v15.1)과 연동 — 함정 ECM이 SEAD 지원."),
            ("v18.4", "높음", "전략 폭격 & 적 기지 타격",
             "B-1B·KF-21·현무-3C(v16.1) 합동 적 항구·비행장·레이더 타격. "
             "타격 성공 시 적 해군 출항 능력 저하 → v17 SLOC 위협 감소. "
             "적 재건 시간 모델. 타격 효과가 캠페인 전체에 누적 반영."),
            ("v18.5", "중간", "공군 작전 캠페인 통합",
             "v17 해군 캠페인 엔진에 공군 임무 큐 추가. "
             "해군 SLOC 위협 상승 시 공군 CAS·SEAD 자동 요청. "
             "합동 교전권(ROE) 구역 중복 처리. "
             "공군 작전 결과 캠페인 보고서(v17.7) 통합."),
            # ── v19.x — 육군 작전급 ────────────────────────────────────────────
            # 선행 필수: v18 공군 작전급 완성
            ("v19.1", "매우 높음", "지상군 전력 DB & 기동 모델",
             "육군 전력 DB: K2 흑표·K21·K9 자주포·천무·패트리엇·사드. "
             "지형 기동 모델: v13.1 3D 지형 기반 이동 속도·시야 계산. "
             "해안 상륙 구역 → 내륙 진출 경로 모델링."),
            ("v19.2", "높음", "지상 방공망 모델",
             "패트리엇·천궁·사드·현무-II 지대지. "
             "지상 방공망이 해군·공군 작전 구역에 미치는 영향. "
             "적 지대함 미사일(DF-21D 등) → 해군 SLOC 위협 연동. "
             "v17 SLOC 모델과 직결: 지상 방공망 손실 시 해군 피해 증가."),
            ("v19.3", "높음", "해상 상륙작전 지원",
             "육군 상륙사단 → 해군 독도함·LST 수송 임무. "
             "상륙 작전: 해군 함포 지원 + 공군 CAS + 육군 상륙 순서 시뮬. "
             "해군 v17 캠페인에 상륙 임무 유형 추가. "
             "교두보 확보 성공 여부 → 작전 목표 달성도에 반영."),
            ("v19.4", "중간", "육군 작전 캠페인 통합",
             "v17·v18 캠페인 엔진에 육군 임무 큐 추가. "
             "지상전 전황 → 해군 SLOC·공군 제공권에 상호 영향. "
             "육군 작전 결과 캠페인 보고서 통합."),
            # ── v20.x — 육해공 통합 합동작전 ──────────────────────────────────
            # 선행 필수: v17·v18·v19 전체 완성
            ("v20.1", "매우 높음", "합동작전 사령부 모델 (JCS)",
             "육해공군 자산을 통합 지휘하는 합동작전 계획 엔진. "
             "자원 충돌 해결: 공군기 1대가 CAS·CAP·SEAD 중 어디에 투입? "
             "합동교전구역(JFACC·JFLCC·JFMCC) 자동 분할·조정. "
             "각 군의 작전 목표 우선순위 충돌 시 합동 최적화."),
            ("v20.2", "높음", "합동 화력 지원 (Joint Fires)",
             "해군 함포·토마호크 + 공군 폭격 + 육군 천무·현무가 "
             "동일 표적에 동시 또는 순차 타격하는 화력 협조 모델. "
             "중복 타격 방지(fratricide 회피) 알고리즘. "
             "화력 효과 누적: 첫 타격 50% 파괴 → 재타격 추가 30% 효과."),
            ("v20.3", "높음", "합동 작전 시나리오 라이브러리",
             "육해공 통합 시나리오 3종: "
             "① 한반도 전면전 개전 72시간 — 북한 전면 공세 대응 "
             "② 대만해협 위기 — 중국 상륙 저지 한미 합동 대응 "
             "③ 독도·이어도 분쟁 — 해상·공중 제한전 "
             "각 시나리오: 초기 전력 배치 + 작전 목표 + 성공 판정 기준."),
            ("v20.4", "중간", "합동작전 통합 보고서",
             "육해공 전 전력의 캠페인 결과를 통합 보고서로 출력. "
             "군별 기여도 분석: 해군이 SLOC 방어에 얼마나 기여? 공군 SEAD가 해군 손실을 얼마나 줄였나? "
             "전역 비용·손실 대비 목표 달성률. 합동작전 최적 전력 구성 추천."),
        ]
