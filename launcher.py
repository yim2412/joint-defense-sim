"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v10.21 — PyQt6 런처                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v10.21 — 전 로드맵(v11~v20) 기둥(아키텍처 핵심) 구체화]                   ║
║  UPD-A  _PLANS 각 항목에 【기둥】추가 — 클래스명·필드·연동 지점 명시         ║
║  UPD-B  v11~v16 전술급 + v17 해군작전급 기둥 35개 항목                      ║
║  UPD-C  v18~v20 공군·육군·합동작전 기둥 13개 항목                           ║
...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v11.1", "낮음", "검증 시뮬 실행",
             "기준 시나리오(기동전단 기본 vs 랴오닝 항모전단) MC 500회. "
             "요격률·피격·비용 변화량 분석. 이상값 발생 시 원인 추적 후 재조정. "
             "【기둥】이후 모든 버전의 비교 기준(baseline) 숫자 확정."),
            ("v11.2", "낮음", "차기 계획 수립",
             "v12~v16 항목 우선순위 최종 합의. 구현 순서·의존성 정리. "
             "【기둥】각 버전 간 선행 조건 명시 → 개발 순서 고정."),
            ("v11.3", "중간", "설정 패널 UI 전면 정리",
             "기본/환경/방어전술/항공자산/고급 5탭으로 분리. "
             "체크박스 툴팁 통일 · 기본값 표시 · 프리셋 빠른 적용 버튼. "
             "【기둥】5탭 구조 확정 → v12~v16 신규 항목이 이 구조에 삽입됨."),
            ("v11.4", "중간", "MC 성능 최적화",
             "MC 10,000회 이상 고속 실행 목표. "
             "① SimV7 핫패스 numpy 벡터화 ② SimFrame 기록 MC 모드 비활성화 ③ LHS sobol_npp=3→5. "
             "【기둥】v14.2 ML 학습 데이터 생성 속도 확보 + v17 캠페인 MC 기반."),
            ("v11.5", "낮음", "시나리오 라이브러리",
             "교리 기반 시나리오 4종: 서해 차단작전·독도 방어·항모전단 요격·북한 포화도발. "
             "cfg JSON + 설명 + 권장 편성 포함. "
             "【기둥】시나리오 표준 포맷 확정 → v17~v20 캠페인 시나리오 포맷 재사용."),
            # ── v12.x — 물리 엔진 고도화 ──────────────────────────────────────
            # 기둥: MissileObj 물리 상태 추가 + 탐지 모델 방정식화
            ("v12.1", "매우 높음", "실제 유도 알고리즘 (PNG)",
             "현재 즉시 Pk 판정 → 비례항법(PNG) 실제 물리 유도. "
             "미사일이 매 tick 표적 방향으로 회전 추격, 회피 기동 실제 반영. "
             "【기둥】MissileObj에 heading·velocity_vec·normal_accel 추가. "
             "기존 pk_base는 PNG 회피 실패 시에만 적용 → 이중 판정 구조."),
            ("v12.2", "매우 높음", "수중 음향 전파 모델",
             "단순 소나 탐지거리 → SONAR 방정식(TL·NL·DT·SL) 완전 구현. "
             "WOA18 수온으로 실제 음파 굴절 경로 계산. 수렴 지대·음영 구역 구분. "
             "【기둥】_detect_range_m()에 카테고리별 방정식 분기 추가. "
             "대잠 탐지가 '거리' 아닌 '물리 계산값'으로 대체됨."),
            ("v12.3", "매우 높음", "함정 생존 능력 동적 모델링",
             "현재 정적 계수 감소 → 침수 속도·복원력(GZ곡선)·격실 배수 시뮬. "
             "피격 위치(수선하/수선상)·함종별 격실 배치 반영. 침몰 타임라인 실시간 표시. "
             "【기둥】FriendlyShipObj에 flood_rate·stability_gm·compartment_hp 추가. "
             "alive 판정이 hp≤0에서 stability_gm≤0 또는 flood_rate 초과로 확장."),
            ("v12.4", "중간", "동적 기상 변화",
             "시뮬 진행 중 태풍 접근, 날씨 시간 변화. "
             "기상청 계절별 패턴 기반 확률적 날씨 전이. "
             "【기둥】WEATHER_DB를 정적 선택에서 시간 함수 wx(t)로 교체. "
             "SimV7 run() 루프 내 매 N틱마다 wx 갱신 호출."),
            ("v12.5", "중간", "피아식별 오류 (IFF)",
             "IFF 실패 확률 모델. 식별 지연 + 오인식 → 아군 오사 가능성. "
             "혼잡 전장에서 ROE 이행 지연. "
             "【기둥】EnemyThreatObj에 iff_confidence(0~1) 필드 추가. "
             "_friendly_defense()에서 confidence < threshold이면 교전 보류."),
            # ── v13.x — 시각화 & 인터페이스 ──────────────────────────────────
            # 기둥: 렌더링 엔진 분리 + 실시간 데이터 파이프라인 구축
            ("v13.1", "매우 높음", "3D 전장 + 실제 지도",
             "GEBCO 수심·SRTM 지형 로드 → PyQtGraph/VisPy 3D 오버레이. "
             "레이더 커버리지 구 시각화. 미사일 궤적 3D. LatLon(v10.8)과 직결. "
             "【기둥】현재 matplotlib 2D 탭을 VisPy 실시간 뷰어로 교체. "
             "SimFrame 데이터를 렌더러가 구독하는 pub-sub 구조 도입."),
            ("v13.2", "중간", "C4ISR 네트워크 가시화",
             "함정·항공·지상 데이터링크 연결 상태 그래프/트리 표시. "
             "노드 격침 시 연결 자동 단절 → CEC 변화 실시간 시각화. "
             "【기둥】NetworkX 그래프 객체로 링크 상태 관리. "
             "CEC 활성 여부를 엣지 가중치로 표현 → _friendly_defense() 연동."),
            ("v13.3", "중간", "실시간 다중 KPI 대시보드",
             "시뮬 중단마다 15+ 지표 실시간 계산: "
             "탄약 소모 효율·비용당 격침 비율·피아 교환비·방어 포화도. "
             "【기둥】KPIEngine 클래스 신설 — compile() 결과를 입력받아 지표 계산. "
             "TacticalDialog(v10.7)에 KPI 패널 삽입으로 의사결정 지원."),
            ("v13.4", "중간", "위성 정찰 재방문 주기",
             "궤도 SAR/EO 위성 재방문 주기(12~24h) 시뮬. "
             "창 열릴 때만 OTH 목표 지정 가능. "
             "【기둥】SatelliteOrbit 객체: pass_times 리스트 사전 계산. "
             "_enemy_fire()에서 OTH 사격 전 위성 창 열림 여부 체크."),
            # ── v14.x — AI & 자율화 ────────────────────────────────────────────
            # 기둥: TacticalAI 추상 클래스 → MCTS·ML·Rule-based 교체 가능 구조
            ("v14.1", "매우 높음", "적응형 전술 AI (MCTS)",
             "Monte Carlo Tree Search 기반 적 지휘관 AI. "
             "교전 구간마다 아군 편성·피해 분석 후 전술 실시간 전환. "
             "【기둥】TacticalAI 추상 클래스 정의: decide(state) → action. "
             "MCTSAIAgent가 이를 구현. 난이도 슬라이더로 탐색 깊이(iteration) 조정."),
            ("v14.2", "높음", "ML 기반 즉시 예측",
             "MC 결과로 학습한 신경망(sklearn/PyTorch). "
             "설정 바꾸는 즉시 요격률·피해 실시간 업데이트 (MC 없이). "
             "【기둥】v11.4 MC 고속화로 확보한 학습 데이터(10만+ 샘플) 사용. "
             "저장된 모델(.pkl/.pt)을 v17 캠페인 교전 해결기로 직접 재사용."),
            ("v14.3", "높음", "함정별 자율 AI (Battle Management)",
             "각 함정이 독립 AI 에이전트. CEC·지령 없이도 자율 탐지·사격 판정. "
             "기함 격침 시 차순위 함정이 자동 지휘권 인수. "
             "【기둥】FriendlyShipObj에 ai_agent: TacticalAI 필드 추가. "
             "_friendly_defense()에서 CEC 두절 시 각 함정의 ai_agent.decide() 호출."),
            ("v14.4", "매우 높음", "무인기 군집 (Swarm)",
             "UAV·USV·UUV 수십~수백 드론 분산 경로 동시 접근. "
             "개별 요격 필요 → 탄약 급소모. Boids 알고리즘 기반 군집 AI. "
             "【기둥】SwarmObj 클래스 신설 (경량 MissileObj 집합). "
             "Boids 3규칙(분리·정렬·응집)으로 매 tick 위치 업데이트. "
             "_check_hits()에서 Swarm 개별 요격 처리."),
            ("v14.5", "매우 높음", "ISR 드론 군집 (정찰)",
             "고정익 무인정찰기 편대 배치. 탐지 범위 확장 + OTH 유도 정보 제공. "
             "드론 개별 피격·격추. 배치 비용 vs 탐지 이득 트레이드오프. "
             "【기둥】ISRDroneObj 클래스 신설 (FriendlyAircraftObj 파생). "
             "_detect_range_m()에 드론 커버리지 보정값 합산 (CEC와 유사 구조)."),
            # ── v15.x — 전장 도메인 확장 ──────────────────────────────────────
            # 기둥: 탐지·교전 파이프라인에 도메인 레이어 삽입 구조
            ("v15.1", "높음", "전자전 고도화 + ESM 역탐지",
             "재밍 포드·DRFM 기만기·SIGINT. 주파수 대역별 ECM. "
             "적 ESM: 아군 레이더 신호 → 역방향 미사일 발사. "
             "【기둥】EWLayer 클래스: 탐지 파이프라인 중간에 삽입. "
             "_detect_range_m() 반환값을 EWLayer가 후처리(감쇠·기만). "
             "ESM은 아군 레이더 ON 시 적 EventHeap에 '역탐지' 이벤트 추가."),
            ("v15.2", "높음", "다전장 연동 (공군·지상)",
             "공군 SEAD·지상 패트리엇/THAAD 동시 운용. "
             "합동 ROE 계층 — 공군 담당 구역에 함정 SAM 발사 금지. "
             "【기둥】AirDefenseZone 격자 맵: 구역별 담당 군 표시. "
             "_select_defense_wpn()에서 구역 체크 후 교전 허가 여부 결정."),
            ("v15.3", "높음", "사이버전 모듈",
             "적 CEC 패킷 변조 → 함정 오발사. CIC 부분 마비 → 탐지 지연. "
             "아군 반격: 적 SAM 레이더 교란. "
             "【기둥】CyberAttackEvent: 발생 시 cfg의 특정 플래그를 일시 변조. "
             "예: enable_cec를 False로 강제 → CEC 두절 상태로 교전 진행."),
            ("v15.4", "높음", "분산 해양작전 (DMO)",
             "미 해군 Distributed Lethality 교리. "
             "소형 함정 분산 배치 + 개별 타격 → 집중 방어 회피. "
             "【기둥】FLEET_PRESETS에 '분산 배치' 프리셋 추가. "
             "각 PKG·PCC가 독립 strike_inventory 보유 + 자체 MissionAssigner 운용."),
            ("v15.5", "높음", "해안 방어 시설 통합 (C-RAM)",
             "육상 C-RAM·해안 미사일 포대 고정 배치. 기뢰 차단 구역 설정. "
             "【기둥】GroundAsset 클래스 신설 (고정 위치, FriendlyShipObj 유사). "
             "GROUND_ASSETS_DB에 위치·사거리·Pk 정의. "
             "_ashore_defense() 패턴 재사용하여 지상 자산 교전 처리."),
            ("v15.6", "중간", "ECM 좌표 기만",
             "함정 ECM 강화 시 레이더 화면상 표시 위치 ≠ 실제 위치. "
             "적 미사일이 기만 좌표로 유도 → 명중률 저하. "
             "【기둥】FriendlyShipObj에 decoy_pos: Optional[LatLon] 추가. "
             "적 미사일 target이 decoy_pos로 유도 → INTERCEPT_DIST_M 판정 시 실제 pos 사용."),
            # ── v16.x — 군수·미래 전장 ────────────────────────────────────────
            # 기둥: FriendlyShipObj에 자원 제약 필드 추가
            ("v16.1", "높음", "군수·보급 모델",
             "보급함(AOE) 이동·체류 시간 모델링. VLS 고갈 후 귀환·재장전. "
             "연료 소모 → 고속 기동 시 작전 반경 축소. "
             "【기둥】FriendlyShipObj에 fuel_pct·ammo_resupply_t 추가. "
             "fuel_pct < 20% 시 자동 귀항 이벤트. v17 ForcePool의 '수리 중' 상태와 동일 처리."),
            ("v16.2", "높음", "지향성 에너지 무기 (레이저·HPM)",
             "HELIOS 60kW 레이저 + 고출력 마이크로파. 전력 제한 내 발사. "
             "【기둥】FriendlyShipObj에 power_budget_kw 추가. "
             "레이저 발사 시 power_budget_kw 소모 → 일정 시간 후 재충전. "
             "FRIENDLY_DB에 '레이저_HELIOS' 항목 추가 (pk_base는 고도·속도 함수)."),
            ("v16.3", "높음", "멀티플레이어 대전",
             "WebSocket 서버. 한 명 이지스 함대, 다른 한 명 적 항모전단 실시간 대전. "
             "【기둥】SimV7을 서버 사이드로 분리. "
             "플레이어 입력(무기 선택·살보 수)을 TacticalChoice 큐에 삽입. "
             "v10.7 전술 의사결정 모드의 resume_tactical() 패턴 확장 재사용."),
            # ── v17.x — 작전급 시뮬레이터 ─────────────────────────────────────
            # 선행 필수: v11.4(MC성능) · v14.2(ML예측) · v16.1(군수보급)
            # 기둥: CampaignEngine — EventHeap + ForcePool + MissionQueue
            #        SimV7을 EngagementResolver로 래핑
            ("v17.1", "매우 높음", "캠페인 엔진 기반 설계",
             "이벤트 기반 캠페인 아키텍처 설계·구현. "
             "1틱 = 1시간. 72시간 캠페인이 수초 내 완료. "
             "【기둥】CampaignEngine(cfg) → run() → campaign_result. "
             "내부 구조: EventHeap(heapq) + ForcePool(dict) + MissionQueue(deque). "
             "SimV7은 EngagementResolver.resolve(cfg) → result로 래핑. "
             "교전 발생 이벤트만 SimV7 호출, 나머지는 ML 예측(v14.2)으로 처리."),
            ("v17.2", "높음", "전력 관리 모델 (ForcePool)",
             "전 함정 상태 추적: 출항/초계/귀항/수리대기/수리중/재편성. "
             "손상 함정 수리 기간(피해 심각도별 1~14일), 탄약 재보급 시간. "
             "【기둥】ForcePool: {ship_id: ShipStatus} 딕셔너리. "
             "ShipStatus.next_available_t 로 가용 시각 관리. "
             "v16.1 fuel_pct·ammo_resupply_t를 ForcePool 상태에 통합."),
            ("v17.3", "높음", "임무 배정 시스템 (MissionAssigner)",
             "가용 전력을 복수 임무에 자동 배정: 초계·호위·타격·대잠·보급 호위. "
             "우선순위 기반 배정. 임무 충돌 해결. "
             "【기둥】Mission 데이터클래스: type·zone·priority·required_ships. "
             "MissionAssigner.assign(force_pool, mission_queue) → 배정 결과. "
             "그리디 알고리즘 우선, v14.1 MCTS AI로 교체 가능한 인터페이스."),
            ("v17.4", "높음", "정보 불확실성 모델 (Intel)",
             "적 전력 위치를 확률 분포로 관리. "
             "탐지 자산 재방문 주기마다 정보 갱신. "
             "【기둥】IntelDB: {enemy_id: GaussianBelief(lat, lon, sigma)}. "
             "sigma는 마지막 탐지 이후 경과 시간에 비례해 증가. "
             "MissionAssigner가 sigma를 리스크 계수로 반영."),
            ("v17.5", "높음", "해상 교통로 통제 모델 (SLOC)",
             "한반도 주요 SLOC 3개 통제 상태 시뮬. "
             "적 해역 진입 → SLOC 위협도 상승 → 보급 감소. "
             "【기둥】SLOCMap: 격자별 threat_level(0~1) 맵. "
             "1시간 틱마다 적 위치 기반 threat_level 갱신. "
             "threat_level > 0.7이면 해당 SLOC 차단 → v16.1 연료·탄약 감소 이벤트 발생."),
            ("v17.6", "중간", "캠페인 MC 분석",
             "캠페인 전체 N회 반복 → 전역 결과 통계. "
             "72시간 후 잔존 전력·SLOC 통제 성공률·전역 비용. "
             "【기둥】CampaignEngine.run()을 MC 루프에서 N회 호출. "
             "개별 교전은 v14.2 ML 예측으로 대체 → 1,000회 캠페인이 수분 내 완료."),
            ("v17.7", "중간", "캠페인 보고서 & 시각화",
             "전역 타임라인: 시각별 전력 상태·임무 현황·SLOC 변화 그래프. "
             "v13.1 3D 지도에 캠페인 경로 오버레이. Excel/PDF 자동 생성. "
             "【기둥】CampaignResult.to_timeline() → DataFrame. "
             "기존 save_excel_report_v7() 패턴 확장 재사용."),
            # ── v18.x — 공군 작전급 ────────────────────────────────────────────
            # 선행 필수: v17 완성 · v15.2 다전장 연동
            # 기둥: AirForceDB + AirSupremacyZone 격자 맵 → v17 CampaignEngine 연동
            ("v18.1", "매우 높음", "공군 전력 DB & 임무 모델",
             "한국·미국 공군 전력 DB: KF-21·F-35A·F-15K·F-16·B-1B·B-52 등. "
             "임무 유형: CAP·CAS·SEAD/DEAD·전략폭격·ISR. "
             "【기둥】AIR_FORCE_DB 신설 (engine_v18.py). "
             "AirUnit 클래스: sortie_time_s·range_km·payload·base_location. "
             "v17 ForcePool 구조를 공군용으로 복제 → AirForcePool."),
            ("v18.2", "높음", "제공권 통제 모델",
             "공중 우세 구역(ASZ) 시뮬. CAP 밀도·적 방공망 강도 → 통제 확률 계산. "
             "제공권 상실 시 v17 해군 작전 패널티 연동. "
             "【기둥】AirSupremacyZone: 한반도 격자(10km×10km) 맵. "
             "격자별 air_control(0~1) 값. 1시간 틱마다 CAP 출격 수·적 방공 강도로 갱신. "
             "해군 _detect_range_m()에 air_control 보정 적용."),
            ("v18.3", "높음", "SEAD/DEAD 작전 모델",
             "적 방공망(SA-21·HHQ-9·S-400) 탐지·타격 캠페인. "
             "SEAD 성공 시 해군 작전 구역 확대. 방공망 복구 6~48시간. "
             "【기둥】EnemyADSite 클래스: location·hp·recovery_t. "
             "SEAD 임무 실행 → EnemyADSite.hp 감소 → AirSupremacyZone 자동 갱신. "
             "v15.1 EWLayer와 연동: 적 방공망 손상 시 ESM 탐지 범위 감소."),
            ("v18.4", "높음", "전략 폭격 & 적 기지 타격",
             "B-1B·KF-21·현무-3C 합동 적 항구·비행장·레이더 타격. "
             "타격 성공 → 적 해군 출항 능력 저하 → v17 SLOC 위협 감소. "
             "【기둥】EnemyBaseDB: 기지별 capacity·current_hp·rebuild_days. "
             "타격 이벤트 발생 → capacity 비례 감소 → v17 EventHeap에 '적 전력 감소' 이벤트 삽입."),
            ("v18.5", "중간", "공군 작전 캠페인 통합",
             "v17 캠페인 엔진에 공군 임무 큐 추가. "
             "SLOC 위협 상승 시 공군 CAS·SEAD 자동 요청. "
             "【기둥】v17 MissionQueue에 AirMission 타입 추가. "
             "MissionAssigner가 해군·공군 자산을 통합 배정. "
             "v17.7 캠페인 보고서에 공군 임무 타임라인 레이어 추가."),
            # ── v19.x — 육군 작전급 ────────────────────────────────────────────
            # 선행 필수: v18 완성
            # 기둥: GroundUnitDB + TerrainMovementModel → SLOC·제공권과 상호 영향
            ("v19.1", "매우 높음", "지상군 전력 DB & 기동 모델",
             "육군 전력 DB: K2·K21·K9 자주포·천무·패트리엇·사드. "
             "v13.1 3D 지형 기반 이동 속도·시야 계산. 상륙→내륙 진출 경로 모델링. "
             "【기둥】GROUND_UNIT_DB 신설 (engine_v19.py). "
             "TerrainMovementModel: 격자별 terrain_type → speed_factor 룩업. "
             "GroundUnit.advance(terrain_map, dt) → 위치 갱신."),
            ("v19.2", "높음", "지상 방공망 모델",
             "패트리엇·천궁·사드·현무-II. "
             "지상 방공망이 해군·공군 작전 구역에 영향. "
             "적 DF-21D 등 → SLOC 위협 연동. "
             "【기둥】GroundADSite 클래스 (v18 EnemyADSite와 동일 구조, 아군용). "
             "지상 방공망 커버리지를 AirSupremacyZone 격자에 합산. "
             "지상 방공망 손실 → ASZ 값 하락 → 해군 패널티 연쇄 발생."),
            ("v19.3", "높음", "해상 상륙작전 지원",
             "상륙사단 → 독도함·LST 수송. 해군 함포 지원 + 공군 CAS + 육군 상륙 순서. "
             "교두보 확보 성공 여부 → 작전 목표 달성도. "
             "【기둥】v17 MissionQueue에 AmphMission 타입 추가. "
             "AmphMission: 해군 수송 단계 → 공군 CAS 단계 → 육군 상륙 단계 순서 체인. "
             "각 단계 성공 확률 독립 계산 후 곱연산."),
            ("v19.4", "중간", "육군 작전 캠페인 통합",
             "v17·v18 엔진에 육군 임무 큐 추가. "
             "지상전 전황 → SLOC·제공권 상호 영향. "
             "【기둥】CampaignEngine.step()에 GroundPhase 추가. "
             "3개 군(해·공·육) 임무가 동일 EventHeap에서 우선순위로 처리. "
             "상호 영향: 지상 방공망 손실 → ASZ → SLOC 도미노 반영."),
            # ── v20.x — 육해공 통합 합동작전 ──────────────────────────────────
            # 선행 필수: v17·v18·v19 전체 완성
            # 기둥: JCS 자원 충돌 해결 알고리즘 + 통합 목표 함수
            ("v20.1", "매우 높음", "합동작전 사령부 모델 (JCS)",
             "육해공군 자산 통합 지휘. 자원 충돌 해결. "
             "합동교전구역(JFACC·JFLCC·JFMCC) 자동 분할. "
             "【기둥】JCSEngine: v17·v18·v19 CampaignEngine 위에 오케스트레이터 레이어. "
             "ConflictResolver.resolve(missions) → 중복 자산 배정 제거. "
             "목표 함수: SLOC_rate × ASZ_rate × cost_efficiency 최대화."),
            ("v20.2", "높음", "합동 화력 지원 (Joint Fires)",
             "해군 함포·토마호크 + 공군 폭격 + 육군 천무·현무 동일 표적 협조 타격. "
             "fratricide 방지 알고리즘. 화력 효과 누적 반영. "
             "【기둥】JointFiresMission: target + contributor_list(해군·공군·육군 조합). "
             "효과 계산: 1 − ∏(1 − pk_i). "
             "타격 순서(시차 공격) 시 첫 타격 손상도 → 이후 타격 pk_i 보정."),
            ("v20.3", "높음", "합동 작전 시나리오 라이브러리",
             "육해공 통합 시나리오 3종: "
             "① 한반도 전면전 72시간 ② 대만해협 위기 ③ 독도·이어도 제한전. "
             "【기둥】JointScenario 포맷: 각 군 초기 ForcePool + 작전 목표 리스트 + 성공 판정 기준. "
             "v11.5 시나리오 포맷을 합동 버전으로 확장 재사용."),
            ("v20.4", "중간", "합동작전 통합 보고서",
             "육해공 전 전력 캠페인 결과 통합. 군별 기여도 분석. "
             "전역 비용·손실 대비 목표 달성률. 최적 전력 구성 추천. "
             "【기둥】JointCampaignResult: 3개 군 CampaignResult 집계. "
             "기여도 = 각 군의 목표 달성 기여 이벤트 수 / 전체 이벤트 수. "
             "v17.7 보고서 패턴 확장 재사용."),
        ]
