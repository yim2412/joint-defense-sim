"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v8.03 — PyQt6 런처                 ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v8.03 — 향후 계획 정리: 신규 7개 항목 추가 및 우선순위 재정렬]            ║
║  NEW-A  VLS 재고 추적·북한 탄도탄·A/B 비교·비용효과·CEC·AI·대함공격 추가   ║
║  NEW-B  v8.x 항목을 엔진/지휘통제/시나리오 그룹으로 분류, 우선순위 재정렬  ║
║                                                                              ║
║  [v8.00 — 도움말/튜토리얼 탭 추가]                                          ║
...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            # ── v8.x : 엔진 현실성 ───────────────────────────────────────────
            ("v8.x", "낮음", "VLS 재고 추적",
             "각 함정의 VLS 발사관 수를 실제 장착량(세종대왕급 SM-2 32발·SM-3 8발 등)으로 추적. "
             "탄약이 소진되면 해당 무기 교전 불가. 재고 고갈이 방어 실패의 실질 원인으로 반영. "
             "최소 재고 분석 탭과 연동해 '몇 발 남았을 때 임무가 위험해지는가' 수치화."),
            ("v8.x", "낮음", "북한 탄도탄 포화 공격 시나리오",
             "노동·화성-12 계열 탄도미사일 20~40발 동시 발사 시나리오. "
             "SM-3(이지스) → THAAD → PAC-3 다층 요격 체계 순서대로 요격 판정. "
             "한국 해군의 실제 1순위 위협. 레이어별 요격 성공률과 누락 탄두 수 통계 출력."),
            ("v8.x", "낮음", "A/B 편대 구성 비교",
             "두 가지 편대 구성을 동시에 시뮬하고 결과를 나란히 비교. "
             "동일 위협 조건에서 '이지스 3척 편대'와 '이지스 2척 + 호위함 2척' 중 어느 쪽이 나은지 즉시 확인. "
             "전력 기획·예산 분석에 직접 활용 가능."),
            ("v8.x", "낮음", "비용-효과 분석",
             "요격 1건당 소모 비용 계산(SM-3 약 300억, SM-2 약 20억, ASROC 약 5억). "
             "무기 소모량 × 단가 기반 교전 비용 차트. "
             "최적 조합 추천 탭과 연동해 '성능 대비 비용' 최적점 시각화."),
            # ── v8.x : 지휘통제·전술 ─────────────────────────────────────────
            ("v8.x", "중간", "협동 교전 (CEC)",
            ...
            ("v8.x", "중간", "적 전술 AI 강화",
             ...
             "① 채널 포화: 아군 교전 채널 수 × 1.5배를 동시 발사해 방어망 압도. "
             "② 시차 공격: 빠른 미사일로 채널 먼저 소모 → 뒤따르는 순항미사일로 마무리. "
            ...
            ("v9.x", "중간", "아군 공격 임무 (대함 공격)",
             ...
             "중국 항모전단 시나리오에서 아군이 적 항모에 선제 타격 후 보복 공격 받는 공방 흐름 구현."),
