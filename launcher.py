...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v8.x", "낮음", "[C-3] 중국 항모 3종 사진 없음",
             "v8.15에서 추가한 랴오닝·산둥·푸젠 항모가 ENEMY_DB에만 있고 "
             "assets/images/ 폴더에 사진 파일이 없음. "
             "스펙시트에서 선택 시 사진 영역이 빈 칸으로 표시됨. "
             "랴오닝 (항모).jpg / 산둥 (항모).jpg / 푸젠 (항모).jpg 추가 필요."),
            # ── v9.x ────────────────────────────────────────────────────────
            ...
            ("v9.x", "중간", "해협 통과 방어 시나리오",
             "⚠ 지형 모델 선행 필요 — v9.x로 이동. "
             "탐지거리 보정계수를 근거 없는 숫자(×0.65 등)로 구현하면 잘못된 결과. "
             "DEM 지형 연동 완료 후 해협 형태에서 자연스럽게 도출되는 구조로 구현. "
