    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v9.x", "중간", "아군 공격 임무 (대함 공격)",
             "현재 방어 전용에서 공격 임무 추가. "
             "해성·현무-3으로 적 수상함 직접 공격. "
             "중국 항모전단 시나리오에서 아군이 적 항모에 선제 타격 후 보복 공방 흐름 구현."),
            ("v9.x", "높음", "지형·해상 환경 반영",
             "실제 지형 데이터(DEM)를 적용해 산·섬 뒤 레이더 음영 구역 구현. "
             "수온층 데이터 연동으로 음파 탐지 거리가 수온에 따라 달라지는 현실적 대잠 환경 구현. "
             "해협 통과 시나리오의 선행 조건."),
            ("v9.x", "중간", "해협 통과 방어 시나리오",
             "⚠ 지형 모델 선행 필요. "
             "DEM 연동 완료 후 해협 형태에서 탐지거리가 자연스럽게 도출되는 구조로 구현. "
             "이어도·대한해협·쓰시마 해협 프리셋 제공."),
            ("v9.x", "중간", "전자전 강화 (ECM/ECCM)",
             "현재 ECM이 단순 Pk 보정값인데 재밍 vs 항재밍(ECCM) 상호작용으로 확장. "
             "적 전자전기(J-16D)가 아군 레이더를 교란하면 탐지거리 감소, "
             "아군 ECCM이 대응하면 일부 회복. 이후 시나리오 전반에 활용되는 기반 작업."),
            ("v9.x", "중간", "북한 잠수함 선제 기습 시나리오",
             "수중에서 어뢰+해성-3 동시 발사로 수상함 기습하는 시나리오. "
             "현재 대잠 시나리오 공백을 메우는 핵심 위협 추가. "
             "신포급·039형 잠수함 기습 프리셋 및 아군 대잠 대응 흐름 구현."),
            ("v9.x", "낮음", "다층 동시 위협 프리셋",
             "공중 포화 + 수상함 대함미사일 + 잠수함 어뢰가 동시에 들어오는 최악의 시나리오 프리셋. "
             "현재 엔진에서 이미 가능하지만 전용 프리셋이 없음. "
             "복합 위협 대응 훈련용으로 3~4개 프리셋 추가."),
            # ── v10.x ───────────────────────────────────────────────────────
            ("v10.x", "매우 높음", "완전 양방향 교전",
             "_enemy_defense()로 적 수상함의 아군 대함미사일 요격은 이미 구현됨. "
             "남은 것: 적 함정·항공기가 아군 SAM(SM-2·SM-6)을 CIWS/SAM으로 요격하는 로직. "
             "공격과 방어가 동시에 이뤄지는 실제 해전 구현. 엔진 구조 전면 재설계 필요. "
             "v10.x 단독 메이저 작업."),
            ("v10.x", "높음", "적 항공모함 타격 작전",
             "아군이 공격자, 적 항모전단이 방어자로 역전되는 시나리오. "
             "KSS-III 현무-3 + PKG 해성 + 함재기 AGM-84 조합으로 적 항모 격침 시도. "
             "아군 공격 임무(v9.x)의 확장판. 완전 양방향 교전 완성 후가 자연스러운 순서."),
            ("v10.x", "높음", "전술 의사결정 모드 (반자동 시뮬)",
             "매 교전 단계마다 운용자가 무기 선택·발사 결정을 직접 내리는 반자동 모드. "
             "SM-2 vs SM-6 vs ESSM 선택, 살보 수 결정 등 실제 전투 의사결정 훈련 목적. "
             "교육·워게임 시나리오 활용 가치."),
        ]
