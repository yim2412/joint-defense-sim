"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v10.17 — PyQt6 런처                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v10.17 — v11.x 로드맵 완성: v11.1~v11.29 총 29개 항목]                   ║
║  UPD-A  _PLANS: v11.6~v11.29 추가 (PNG유도·3D지도·음향·AI·Swarm·사이버전 등)║
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v11.1", "낮음", "검증 시뮬 실행",
             ...
             "기준 시나리오(기동전단 기본 vs 랴오닝 항모전단) MC 500회 실행 → 요격률·피격·비용 변화량 분석. "
             "이상값(요격률 90%↑ 또는 20%↓) 발생 시 원인 추적 후 수치 재조정."),
            ("v11.2", "낮음", "v11.x 새 계획 수립",
             "v10.x 완료 후 다음 사이클 로드맵 설계. "
             "후보 항목: 지도 오버레이 · 멀티 편대 동시 시뮬 · 실시간 대시보드 · AI 운용 자문. "
             "v11.3~v11.29 우선순위 최종 합의."),
            ("v11.3", "중간", "설정 패널 UI 전면 정리",
             "기능 추가로 복잡해진 설정 패널 재구성. "
            ...
            ("v11.4", "중간", "MC 성능 최적화",
             ...
             "③ LHS sobol_npp=3→5. phase_times 병목 측정 후 상위 2개 구간 집중."),
            ("v11.5", "낮음", "시나리오 라이브러리",
             "실제 교리 기반 시나리오 4종: "
             "서해 차단작전 · 독도 방어 · 동해 항모전단 요격 · 북한 포화도발 대응. "
             "cfg JSON + 설명 텍스트 + 권장 편성 포함."),
            # ── v11.x — 물리 모델 고도화 ──────────────────────────────────────
            ("v11.6", "매우 높음", "실제 유도 알고리즘 (PNG)",
             "현재 즉시 Pk 판정 → 비례항법(Proportional Navigation Guidance) 실제 물리 유도. "
             "미사일이 매 tick 표적 방향으로 회전하며 추격, 회피 기동 실제 반영. "
             "MissileObj에 heading, normal_accel 추가. SAM 회피 가능성 물리 기반 계산."),
            ("v11.7", "매우 높음", "3D 전장 + 실제 지도",
             "GEBCO 수심·SRTM 지형 데이터 로드 → PyQtGraph/VisPy 3D 오버레이. "
             "레이더 커버리지 구(Sphere) 시각화. 미사일 궤적 3D 표현. "
             "LatLon 좌표계(v10.8)와 직결, 함정·미사일 실제 지리 위치 표시."),
            ("v11.8", "매우 높음", "수중 음향 전파 모델",
             "현재 단순 소나 탐지거리 → SONAR 방정식(TL·NL·DT·SL 전체). "
             "WOA18 수온 프로파일로 실제 음파 굴절 경로 계산. "
             "수렴 지대(Convergence Zone), 음영 구역, 직접 경로 구분. "
             "잠수함 탐지 확률이 수심·해역·계절에 따라 물리적으로 결정."),
            ("v11.9", "매우 높음", "적응형 전술 AI (MCTS)",
             "Monte Carlo Tree Search 기반 적 지휘관 AI. "
             "매 교전 구간 아군 편성·피해 분석 후 전술 실시간 전환: "
             "포화공격 → 분산 접근 → 기만 전술 순 적응. "
             "난이도 슬라이더로 AI 깊이(탐색 횟수) 조정."),
            ("v11.10", "매우 높음", "무인기 군집 (Swarm)",
             "UAV·USV·UUV 자율 군집 공격. 수십~수백 드론이 분산 경로 동시 접근. "
             "개별 요격 필요 → 탄약 소모 급증. 드론 군집 AI: Boids 알고리즘 기반. "
             "아군 무인정찰기(EO/SAR) 배치 → 탐지 범위 확장 전술."),
            # ── v11.x — 전장 확장 ─────────────────────────────────────────────
            ("v11.11", "높음", "전자전 고도화",
             "재밍 포드, DRFM 기만기, SIGINT 탐지. "
             "현재 단순 ECM 배율 → 주파수 대역별 세부 모델. "
             "적 레이더 역탐지(ESM): 아군 레이더 신호 → 적이 감지 → 역방향 미사일 발사. "
             "레이더 ON/OFF 타이밍이 핵심 전술 결정점으로 격상."),
            ("v11.12", "높음", "다전장 연동",
             "공군 SEAD·지상 패트리엇/THAAD·이지스어쇼어 동시 운용. "
             "합동작전 교전권(ROE) 계층 — 함정은 공군이 담당하는 구역에 SAM 발사 안 함. "
             "지상·공중·해상 자산 통합 방공 구역 시각화."),
            ("v11.13", "높음", "군수·보급 모델",
             "보급함(AOE/AO) 이동·체류 시간 모델링. "
             "VLS 고갈 후 항구 귀환·재장전 시간 시뮬. "
             "연료 소모: 고속 기동 시 작전 반경 축소. "
             "장시간 작전에서 '언제 후퇴해서 재보급할지' 전술 판단점 추가."),
            ("v11.14", "높음", "멀티플레이어 대전",
             "WebSocket 서버 기반. 한 명은 이지스 함대 지휘, 다른 한 명은 적 항모전단. "
             "각 플레이어가 실시간으로 무기 선택·발사·회피 기동 입력. "
             "관전 모드 + 대전 결과 기록 시스템."),
            ("v11.15", "높음", "ML 기반 즉시 예측",
             "MC 결과로 학습한 신경망(sklearn/PyTorch). "
             "설정 바꾸는 즉시 요격률·피해 예측값 실시간 업데이트 (MC 없이). "
             "학습 데이터: 편대·위협·날씨·옵션 조합 → 요격률/피격/비용. "
             "모델 재학습 버튼 → 새 DB 수치 반영."),
            ("v11.16", "중간", "동적 기상 변화",
             "시뮬 진행 중 태풍 접근, 날씨 시간 변화. "
             "날씨 변화에 따른 탐지·교전 능력 실시간 변동. "
             "기상청 계절별 기상 패턴 기반 확률적 날씨 전이 모델."),
            ("v11.17", "중간", "피아식별 오류 (IFF)",
             "IFF(피아식별장치) 실패 확률 모델. "
             "식별 지연(초기 탐지 후 N초) + 오인식 → 아군 오사 가능성. "
             "혼잡 전장에서 교전 규칙(ROE) 이행 지연. "
             "실수로 아군기를 적으로 분류하는 실전 상황 재현."),
            ("v11.18", "중간", "위성 정찰 재방문 주기",
             "궤도 SAR/EO 위성의 재방문 주기(12~24시간) 시뮬. "
             "위성 창 열릴 때만 대함 미사일 OTH 목표 지정 가능. "
             "위성 공백 시간 → 아군 기습 기회 창. "
             "정찰 위성 궤도 파라미터 → 창 시간 자동 계산."),
            ("v11.19", "높음", "분산 해양작전 (DMO)",
             "미 해군 분산 치사(Distributed Lethality) 교리 구현. "
             "소형 함정 분산 배치 + 개별 타격 → 집중 방어 회피. "
             "PKG·PCC 군집이 독립 공격망으로 운용. "
             "네트워크 중심 방어에서 분산 다점 공격으로 전환."),
            ("v11.20", "높음", "사이버전 모듈",
             "적 사이버 공격: CEC 데이터링크 패킷 변조 → 함정 오발사. "
             "CIC 시스템 부분 마비 → 탐지 지연 급증. "
             "아군 사이버 반격: 적 SAM 레이더 화면 교란. "
             "사이버전 강도 슬라이더 + 대응 패치 시간 변수."),
            # ── v11.x — 신규 발굴 아이디어 ────────────────────────────────────
            ("v11.21", "매우 높음", "함정 생존 능력 동적 모델링",
             "현재 서브시스템 피해는 정적(계수 감소). "
             "침수 속도·복원력(GZ곡선)·격실 배수 시뮬로 침몰 타임라인 예측. "
             "피격 위치(수선하/수선상) + 함정 종류별 격실 배치 반영. "
             "침몰 예상 시간을 전투 중 실시간 표시."),
            ("v11.22", "매우 높음", "해상 초계 드론 군집 (ISR UAV)",
             "고정익 무인기(RQ-4 유사) 배치 → 탐지 범위 확장 + 미사일 유도 정보 제공. "
             "드론 개별 피격·격추 시스템. 드론 배치 비용 vs 탐지 이득 트레이드오프. "
             "전술 의사결정 모드(v10.7)에서 드론 운용 선택지 추가."),
            ("v11.23", "높음", "적 역탐지 체계 (ESM)",
             "아군 레이더/소나 신호 → 적 ESM(전자지원수신)으로 방향·세기 파악. "
             "적이 탐지되기 전 먼저 역방향 미사일 발사 가능. "
             "레이더 ON/OFF 전술(v10.x ARM 방어)보다 근본적 위협 — "
             "레이더 자체가 위치 노출 수단임을 모델링."),
            ("v11.24", "높음", "함정별 자율 AI (Battle Management)",
             "각 함정이 독립 AI 에이전트. CEC·지령 없이도 탐지·사격 판정 자율 수행. "
             "기함 격침 시 차순위 함정이 자동으로 편대 지휘권 인수. "
             "자율화 수준 슬라이더: 완전 수동 ~ 완전 자율."),
            ("v11.25", "높음", "해안 방어 시설 통합 (C-RAM)",
             "육상 C-RAM·지상 레이더·해안 미사일 포대 고정 배치. "
             "연안 접근 적 미사일·드론 대응. 기뢰 차단 구역 설정. "
             "한반도 지형 연동: 인천·부산·동해 항만 방어 시나리오."),
            ("v11.26", "중간", "함정 위장 기동 (ECM 좌표 기만)",
             "아군 함정이 ECM 강화 선택 시 레이더 화면상 위치를 실제와 다르게 표시. "
             "적 미사일이 기만 좌표로 유도 → 명중률 저하. "
             "현재 탐지거리 감소 ECM과 별개 — 위치 기만 레이어 추가."),
            ("v11.27", "중간", "C4ISR 네트워크 가시화",
             "함정·항공·지상 기지 간 데이터링크 연결 상태를 그래프/트리 표시. "
             "특정 노드(함정) 격침 시 연결 자동 단절 → CEC 변화 실시간 시각화. "
             "네트워크 취약점 분석: 어떤 함정 먼저 격침하면 방어망 붕괴하는지 전략 계산."),
            ("v11.28", "중간", "실시간 다중 KPI 대시보드",
             "시뮬 중단 구간마다 15+ 성능지표 실시간 계산: "
             "탄약 소모 효율 · 단위 비용당 격침 비율 · 피아 교환비 · 방어 포화도. "
             "전술 의사결정 모드(v10.7)와 시너지 — 무기 교체 판단 근거 즉시 제공."),
            ("v11.29", "높음", "지향성 에너지 무기 (레이저·HPM)",
             "근미래 방어 시스템: 고출력 레이저(HELIOS 60kW) + 고출력 마이크로파(HPM). "
             "무한 탄약이지만 전력 제한 (함정 발전기 용량 내에서만 연속 발사). "
             "드론 군집(v11.22)·UAV에 특히 효과적. 현무-4 초음속 단계는 레이저 무력화."),
        ]
