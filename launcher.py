...
_FEATURES = [
    ("🗺️  전장 애니메이션",
     "전투 상황을 위에서 내려다보는 화면으로 재생. 마우스 휠·버튼으로 확대/축소, "
     "재생 속도 0.5×~4× 조절, 격추·피격 장면으로 바로 이동, 화면 캡처 저장 가능."),
    ("📊  반복 시뮬레이션 통계",
     "같은 시나리오를 수백~수천 번 자동 반복해 '평균적으로 몇 %를 막아내는지' 확률로 계산. "
     "결과가 운에 따라 얼마나 달라지는지 분포 그래프로 확인 가능."),
    ("✅  성능 기준 자동 판정",
     "한국 해군이 요구하는 성능 기준 8가지(반응 시간·요격 성공률·함정 생존율 등)를 "
     "시뮬레이션 결과로 자동 통과/실패 판정. 어떤 기준이 미달인지 한눈에 확인."),
    ("🌤️  날씨 조건 비교",
    ...
    ("📡  레이더 포화도",
     "각 함정이 동시에 추적·유도할 수 있는 표적 수가 시간대별로 얼마나 찼는지 색으로 표시. "
     "빨간색에 가까울수록 꽉 찬 상태 — 가득 차면 추가 요격 불가."),
    ("💰  격추 비용 효과",
     "적 1기를 격추하는 데 평균 얼마의 비용이 들었는지 달러 단위로 표시. "
     "무기별 남은 재고도 함께 확인 가능."),
    ("🔫  탄약 소모 현황",
     "반복 시뮬레이션 기준 평균 남은 탄약을 무기별 가로 막대그래프로 표시. "
     "어떤 무기가 가장 많이 소모됐는지 비교."),
    ("📈  요격률 범위 추정",
     "요격률이 실제로 몇 %~몇 % 사이에 들어올지 통계적으로 추정해 표시. "
     "함정별 평균 피격 횟수 분포도 함께 표시."),
    ("⏱️  교전 타임라인",
     "어느 시점에 어떤 교전이 발생했는지 시간축 위에 점으로 표시. "
     "언제 가장 많은 위협이 몰렸는지, 요격이 집중된 구간이 어딘지 한눈에 파악."),
    ("🌪️  설정 감도 분석",
     "재고 수량·탐지 가능 거리·날씨 등 설정을 하나씩 바꿔가며 "
     "요격률이 얼마나 달라지는지 분석. 백그라운드 실행이라 다른 작업에 영향 없음."),
    ("🧭  방향별 취약점",
     "북·남·동·서 각 방향에서 공격이 들어올 때 피격률과 요격률이 "
     "어떻게 달라지는지 방사형 그래프로 표시 — 어느 방향이 가장 취약한지 확인."),
    ("🎯  성능 기준 충족률",
     "8가지 성능 기준 각각을 얼마나 충족했는지 방사형 그래프로 한눈에 비교. "
     "전체적인 방어 역량의 균형이 잡혀 있는지 시각적으로 파악."),
    ("📊  위협 유형별 요격률",
     "항공기·탄도미사일·순항미사일·잠수함 유형별로 각각 몇 %를 막아냈는지 분류해서 표시. "
     "어떤 유형의 위협에 취약한지 파악 가능."),
    ("⏰  취약 시간대 분석",
     "교전 시작 후 몇 초 구간에 피격이 집중됐는지 그래프로 표시. "
     "가장 위험한 시간대를 빨간색으로 강조해 방어 집중 구간을 한눈에 파악."),
    ("🔄  이전 결과 비교",
    ...
    ("📄  보고서 내보내기",
     ...
     "요격률·비용·교전 통계·그래프가 모두 포함된 분석 보고서 형태."),
    ("🖥️  시스템 모니터",
    ...
    ("⚡  멀티코어 병렬 처리",
     "프로그램 시작 시 미리 여러 개의 처리 담당자를 준비해 두고, 반복 시뮬레이션 시 "
     "최대 8개 CPU 코어를 동시에 활용해 빠르게 처리. 코어가 많을수록 속도 향상."),
    ("⚙️  현실적 전술 기동",
     "통신 두절 상황, 함정 회피 기동, V자 편대 진형, 포위 공격, "
     "다방위 동시 공격 등 실제 해전에서 쓰이는 전술 상황을 시뮬레이션에 반영."),
    ("🌊  적군 위협 데이터베이스",
     "중국 해군·공군 함정·전투기, 북한 탄도·순항·잠수함발사 미사일, "
     "러시아 극초음속 무기, 신형 대함미사일, 드론 떼·소형 자폭 드론까지 총 43종 위협 수록."),
    ("⚓  한국 해군 함정 (10종+)",
     "이지스 구축함(세종대왕급·정조대왕급)·구축함·호위함 3종 외 "
     "고속정·초계함·상륙함·군수지원함까지 수록. "
     "함정별 실제 무장 구성과 탑재 가능 무기 차이 반영."),
    ("🏴  편대 프리셋 (10종)",
     "한국 해군 실제 편대 구성을 반영한 프리셋 5종 추가: 이지스 기동전단 / 이지스 기동전단(강화) / "
     "독도함 상륙전단 / 동해 해역방어(1함대) / 서해 해역방어(2함대). "
     "기존 5종(단독·기본·탄도탄방어·대잠·최대)과 합쳐 총 10종 프리셋 제공."),
]
