    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v9.x", "낮음", "다층 동시 위협 프리셋",
             "엔진은 이미 공중+수상+수중 동시 처리 가능. 전용 프리셋만 없음. "
             "항공 포화 + 수상함 대함미사일 + 잠수함 어뢰가 동시에 들어오는 최악 시나리오 "
             "3~4개 프리셋 추가. 기존 ENEMY_FLEET_PRESETS에 항목 추가만 하면 됨."),
            ("v9.x", "낮음", "야간·악천후 복합 시나리오",
             "기존 날씨 시스템 위에 시간대(주간·야간·새벽) 조합 추가. "
             "야간: 광학 탐지 불가 + IR 탐지 강화. 폭풍우 야간: 레이더·광학 모두 저하. "
             "다층 동시 위협과 조합 시 최고 난이도 시나리오 구성 가능."),
            ...
            ("v9.x", "중간", "북한 잠수함 선제 기습 시나리오",
             ...
             "② 잠수함 기습 전용 프리셋 (신포×2 + 039×3 등) "
             "③ 기습 시 아군 대잠 대응 흐름 강화."),
            ("v9.x", "중간", "아군 함재 헬기 대잠 현실화",
             "현재 헬기가 청상어 투하까지 교전 흐름은 구현됨. "
             "디핑소나 탐지 → 호버링 → 투하 시간 지연 모델 추가. "
             "수온층 보정과 연계해 탐지 실패·재탐색 흐름 구현. 잠수함 기습 시나리오와 묶어서 구현."),
            ("v9.x", "낮음", "P-8A 포세이돈 해상초계기",
             "현재 P-3C 오라이온 있음. P-8A 추가로 대잠 탐지 능력 현실화. "
             "소노부이 투하 탐지 범위 P-3C 대비 +3km 확대. "
             "Mk.46 5발 탑재. 잠수함 기습 시나리오와 묶어서 구현."),
            ("v9.x", "중간", "전자전 강화 (ECM/ECCM)",
             "현재 단방향: 아군 ECM이 적 미사일 Pk만 감소 (ECM_REF_RANGE_M 구현됨, 30%). "
             "남은 것: ① 적 ecm_power 필드 추가 (J-16D 등 전자전기 재밍 출력) "
             "② 적 ECM → 아군 탐지거리 감소 역방향 로직 "
             "③ 아군 ECCM → 재밍 일부 회복 대응 로직."),
            ("v9.x", "낮음", "채프·플레어·기만체계 세분화",
             "현재 self_defense_pk 하나로 뭉뚱그려진 것을 세분화. "
             "채프(레이더 교란)·플레어(IR 유인)·DRFM(디지털 기만) 각각 독립 확률 적용. "
             "전자전 강화와 묶어서 구현하면 자연스러움."),
