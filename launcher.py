"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v10.28 — PyQt6 런처                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v10.28 — 향후 계획에 '🔧 정비' 항목 추가]                                 ║
║  NEW-A  _PLANS '🔧 정비' 행 — git 추적 안 된 파일 정리 (DB 3종 등록 필요)   ║
║  NEW-B  확인: 해양·지형 DB 3종은 엔진 사용 중, 군사배치 DB는 미사용         ║
...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("🔧 정비", "낮음", "git 추적 안 된 파일 정리",
             "해양·지형 DB 3종(음향·환경·지형)은 엔진이 실제 사용 중인데 git에 미등록 → 등록 필요. "
             "한반도 군사배치 DB는 현재 어디서도 사용 안 함 → 사용/삭제 판단. "
             "이전 세션부터 삭제 표시된 잔존 파일(폐기 애니메이션·대시보드·구버전 보고서)은 폐기 확정 또는 복원 결정."),
            # ── v11.x — 기반 정비 ─────────────────────────────────────────────
            # 기둥: 기준값 확정 → UI 구조 확정 → MC 속도 확보
            ("v11.1", "낮음", "검증 시뮬 실행",
             "기준 시나리오(기동전단 기본 vs 랴오닝 항모전단) 몬테카를로 500회. "
             "요격률·피격·비용 변화량 분석. 이상값 발생 시 원인 추적 후 재조정. "
             "이후 모든 버전의 비교 기준이 됨."),
            ("v11.2", "—", "차기 계획 수립 (완료)",
             "로드맵 상세 문서로 대체 완료 — 48개 항목을 구현·군사·범위 3가지로 검토. "
             "난이도 보정 9건·군사 교정 6건·재배치 4건 반영."),
            ("v11.3", "중간", "설정 패널 화면 정리",
             "기본/환경/방어전술/항공자산/고급 5개 묶음으로 분리. "
             "설정 설명 통일·기본값 표시·프리셋 빠른 적용 버튼. "
             "이후 신규 기능이 이 5개 묶음에 들어감."),
            ("v11.4", "높음", "분석 속도 향상",
             "몬테카를로 분석을 10,000회 이상 빠르게 실행. "
             "1단계(2일): 불필요한 기록 최소화로 2~3배. "
             "2단계(2~3주): 내부 연산 최적화로 추가 5~10배(대규모 재작성, 검증 필수). "
             "작전급 캠페인 분석의 토대."),
            ("v11.5", "낮음", "시나리오 라이브러리",
             "교리 기반 시나리오 4종: 서해 차단작전·독도 방어·항모전단 요격·북한 포화도발. "
             "설정·설명·권장 편성 포함. 작전급 시나리오의 표준 형식이 됨."),
            ("v11.6", "중간", "과거 패치 내역 용어 정리",
             "① 함수명·클래스명·변수명 제거(169개 항목, 주로 v7~v8). "
             "② 용어 통일 — 전체 667항목 검토해 길게 풀어 쓴 설명을 정확한 군사용어로 조정(예: '근접 방어 화기'→'CIWS'). "
             "코드명은 빼되 군사용어는 적극 사용. 일괄 변환 후 검수·빌드."),
            # ── v12.x — 물리 엔진 고도화 ──────────────────────────────────────
            ...
            ("v12.2", "매우 높음", "수중 음향 전파 모델",
             "단순 소나 탐지거리 → 실제 음향 전파(전달손실·주변소음·표적강도) 기반 탐지. "
             "최소 적용은 수심대별 보정 + 수온약층 강화부터. "
             "【현실성】수렴지대는 심해(1000m 이상)인 동해만 가능, 서해(평균 44m)는 불가 → 해역 수심별 분기 필수."),
            ...
            ("v15.1", "높음", "전자전 고도화 + 역탐지",
             "재밍 포드·기만기·신호정보 수집. 주파수 대역별 전자방해. "
             "적 전자지원장비가 아군 레이더 신호를 잡아 역방향 미사일 발사. "
             "【현실성】레이더 방사 = 위치 노출이 현대 해전 핵심 딜레마. 대방사미사일 방어의 근본 확장."),
            ("v15.2", "높음", "다전장 연동 (공군·지상)",
             "공군 SEAD·지상 패트리엇/THAAD 동시 운용 + 합동 교전규칙 구역. "
             "【재배치】사실상 작전급 요소 → 공군 작전급(v18)으로 흡수 권장(전술급 700초엔 반쪽)."),
            ("v15.3", "중간", "사이버전 모듈",
            ...
            # ── v16.x — 군수·미래 전장 ────────────────────────────────────────
            ("v16.1", "중간", "군수·보급 모델",
             "보급함 이동·체류 + 미사일 소진 후 귀환·재장전 + 연료 소모. "
             "【재배치】전술급 700초엔 효용 미미 → 작전급 전력 관리(v17.2)로 흡수 권장(작전급에서 진가)."),
            ...
            # ── v17.x — 작전급 시뮬레이터 ─────────────────────────────────────
            # 선행 필수: v11.4(분석속도) · v14.2(즉시예측) · v16.1(군수보급)
            ...
            ("v17.2", "높음", "전력 관리 모델",
             ...
             "연료·탄약 제약(v16.1)을 여기에 통합. 가용 전력 = 전체 − 수리중 − 귀항중."),
            ("v17.3", "높음", "임무 배정 시스템",
