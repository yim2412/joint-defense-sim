"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v7.16 — PyQt6 런처                 ║
╠══════════════════════════════════════════════════════════════════════════════╣
...
_FEATURES = [
    ...
    ("🌊  적군 위협 데이터베이스",
     ...
     "러시아 극초음속 무기, YJ-21·CM-302 신형 미사일, 드론 떼·수중 자폭 드론까지 총 43종 위협 수록."),
    ("⚓  한국 해군 함정 DB (8종)",
...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ("v7.7", "중간", "결과 차트 UI 프리즈 완전 해결",
             "시뮬레이션 결과 탭을 클릭할 때 프로그램이 잠시 멈추는 현상 완전 해결. "
             "결과 차트 14개를 백그라운드 스레드에서 미리 그려 이미지로 교체하는 방식으로 전환. "
             "탭 전환 즉시 '렌더링 중...' 표시 → 완료 시 이미지 표시."),
            ("v7.7", "중간", "전면 코드·UI 감사",
             "엔진·런처 전체를 처음부터 다시 점검해 숨어 있는 버그, 수치 오류, "
             "미구현 기능을 모두 찾아 수정. 동시에 글자 잘림·여백 부족·초기값 어색한 부분 등 "
             "화면 레이아웃 전반도 함께 정리."),
            ("v7.8", "낮음", "적 무기 DB 확장",
             "새로운 위협 3종 추가: 중국 YJ-21 극초음속 대함미사일(Mach 10+, 1,500km), "
             "CM-302 초음속 순항미사일(Mach 3, 290km), "
             "북한 해성-3 잠수함 발사 순항미사일(1,500km+)."),
            ("v7.8", "낮음", "시뮬레이션 재현 기능 (시드 고정)",
             "같은 시드 번호를 입력하면 몇 번을 실행해도 동일한 결과가 나오도록 설정. "
             "특이한 결과가 나왔을 때 원인을 분석하거나, 조건을 바꿔가며 반복 실험할 때 유용. "
             "설정 화면에 체크박스·입력란 추가, 결과 화면에 사용된 시드 번호 표시."),
            ("v7.8", "중간", "아군 함정 위치 랜덤 배치",
            ...
            ("v7.11", "낮음", "적군 DB / 아군 DB 탭 신설",
             ...
             "아군 무기 10종과 함정 8종도 별도 탭으로 제원 열람 가능."),
            ("v7.11", "중간", "한국 해군 함정 대규모 추가 + 현실 기반 프리셋",
             "수상함 5종 추가 — PKG 윤영하급(유도탄 고속함), PCC 포항급(초계함), "
             "PKX-B 참수리-II(유도탄 고속정), LPH 독도함급(강습상륙함), AOE 소양함(보급함). "
            ...
            ("v7.x", "중간", "DB 탭 상세 스펙시트 패널",
             ...
             "사진 파일이 있으면 함께 표시, 없으면 분류 아이콘으로 대체. 전체 데이터 수록."),
            ("v7.x", "낮음", "DB 사진 자동 수집 스크립트",
             "적군·아군 전 유닛(63종+) 사진을 자동으로 내려받는 download_images.py 작성. "
             "Wikipedia Commons·미 해군 DVIDS 등 저작권 없는 공개 자료 URL만 사용. "
             "스크립트 실행 한 번으로 assets/images/ 폴더에 전 유닛 사진 자동 저장. "
             "스펙시트 패널 완료 후 실행하면 즉시 사진이 연동됨."),
            ("v7.x", "중간", "도움말 / 튜토리얼 탭",
             "프로그램 처음 사용자도 바로 이해할 수 있도록 탭 추가. "
             "Pk·RCS·요격 채널·REQ 등 주요 용어 설명, 시뮬레이션 실행 순서 단계별 안내, "
             "자주 묻는 질문(FAQ) 포함. 처음 실행 시 이 탭이 자동으로 열림."),
            ("v7.x", "중간", "실행 기록 DB 저장",
             "시뮬레이션을 실행할 때마다 날짜·설정·결과를 자동으로 기록 파일에 저장. "
             "결과 탭에서 과거 기록을 조회하고 설정을 불러오거나 결과를 비교 가능. "
             "CSV 파일로 내보내기 지원."),
            ("v7.x", "중간", "REQ 달성 최소 미사일 재고 계산",
             "요구 조건을 충족하려면 미사일이 최소 몇 발 필요한지 자동으로 역산. "
             "'SM-2 최소 24발, 청상어 최소 8발' 형태로 결과 표시. "
             "민감도 분석 탭과 연동해 재고 수량별 달성 확률 곡선도 함께 표시."),
            ("v7.x", "중간", "자동 취약점 진단",
             "MC 결과를 분석해 약점을 자동으로 찾아 알려주는 기능. "
             "예: '순항미사일에 대한 요격률이 REQ 기준 미달', '동시 교전 채널 자주 포화', "
             "'SM-6 재고를 12→18발로 늘리면 REQ 달성' 같은 개선 제안 자동 생성."),
            ("v7.x", "높음", "최적 무기 조합 추천",
             "총 탑재 발수 제한 내에서 REQ를 달성하는 가장 효율적인 무기 조합을 자동 탐색. "
             "유전 알고리즘 또는 격자 탐색 방식으로 수백 가지 조합을 비교해 "
             "최적 조합과 해당 조합의 요격률 분포를 결과로 표시."),
            ("v7.x", "중간", "드론 떼(Swarm) 전술 세부화",
             "현재 소형 드론은 1기씩 개별 처리. 앞으로는 수십 기가 동시에 몰려오는 "
             "'Swarm 그룹' 단위로 묶어 처리. 다수 드론이 요격 채널을 동시에 소모해 "
             "방어 시스템을 포화시키는 전술 구현."),
            ("v7.x", "중간", "대방사 미사일(ARM) 추가",
             "레이더 전파를 추적해 레이더 자체를 파괴하는 미사일 추가 (러시아 Kh-31P, 미국 AGM-88 HARM). "
             "레이더를 켜면 탐지는 가능하지만 ARM 위협이 증가하고, "
             "끄면 안전하지만 적을 탐지할 수 없는 전술적 딜레마 구현."),
            ("v7.x", "중간", "혼합 공격 시나리오",
             "여러 종류의 위협이 동시에 쏟아지는 복합 시나리오 프리셋 제공. "
             "예: '순항미사일 포화 + 탄도탄 + 드론 떼 동시 출현', "
             "'잠수함 어뢰 + 대함미사일 병행 공격'. 시나리오 선택 UI 추가."),
            ("v8.x", "높음", "중국 항모전단 대응 시나리오",
             "랴오닝·산둥·푸젠 항모전단 편성을 DB에 추가 (항모 + 055형 + 054A형 + 잠수함). "
             "전단 내 함정이 서로를 방어하는 상호 방어권 개념 구현. "
             "대함 공격과 함재기 대공 공격 시나리오 모두 지원."),
            ("v8.x", "높음", "연합 작전 (미 해군 편성 추가)",
             "미 해군 함정 DB 추가: DDG-51(Arleigh Burke), CG-47(Ticonderoga), CVN(항모). "
             "SM-6 Block IB, SM-3 IIA, ESSM Block II 등 미군 무장도 추가. "
             "'한미 연합 편대' 프리셋 제공, 결과 탭에 한국·미국 기여도 분리 표시."),
            ("v8.x", "중간", "해협 통과 방어 시나리오",
             "아군 함정이 해협을 통과하는 동안 방어하는 시나리오. "
             "위협이 특정 방향에서 집중 출현하고, 해협 지형에 따른 레이더 차폐 효과 반영. "
             "이어도·대한해협·쓰시마 해협 프리셋 제공."),
            ("v8.x", "중간", "피해 연동 시스템 세분화",
             "현재는 함정 내구도(HP)가 단순히 줄어드는 방식. "
             "앞으로는 레이더·추진·무장이 각각 독립적으로 피해를 입는 방식으로 세분화. "
             "예: 레이더 피탄 → 탐지거리 감소, 추진 피탄 → 속도 감소, 무장 피탄 → 해당 무기 사용 불가."),
            ("v8.x", "높음", "레이더 다중 모드 구분",
             "현재는 탐지·추적·사격통제 레이더가 하나의 탐지거리 값으로 통합. "
             "앞으로는 탐색 레이더(장거리·저정밀) / 추적 레이더(중거리) / 사격통제(단거리·고정밀)로 분리. "
             "탐색→추적→교전 단계별 처리 지연시간 추가로 현실적 반응 사이클 구현."),
            ...
            ("v8.x", "높음", "지형·해상 환경 반영",
             ...
             "'레이더 음영 지역' 구현. 바닷속 수온층 데이터를 연동해 수온에 따라 "
