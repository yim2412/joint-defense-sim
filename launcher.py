"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   이지스 기동전단 통합 방어 시뮬레이터  v10.22 — PyQt6 런처                ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v10.22 — 로드맵 48개 항목 3차원(구현·군사·범위) 현실성 검토]              ║
║  NEW-A  로드맵_상세_v11-v20.md — 항목별 공수·위험·MVP·군사교정·범위 판정    ║
║  UPD-A  _PLANS 난이도 보정 9건 (v11.4↑·v13.3↓·v15.3↓·v16.1↓·v16.3↑ 등)    ║
║  UPD-B  군사 현실성 교정 — PNG g한계·음향 CZ동해한정·레이저 대상제한        ║
║  UPD-C  재배치 권장 표기 — v15.2/v16.1→작전급 흡수, v11.2 완료 처리         ║
...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v11.2", "—", "차기 계획 수립 (완료)",
             "로드맵_상세_v11-v20.md 문서로 대체 완료 — 48개 항목 3차원(구현·군사·범위) 검토. "
            ...
            ("v11.4", "높음", "MC 성능 최적화",
             "MVP-A(2일): SimFrame MC모드 비활성화+배치튜닝 → 2~3배. "
             "MVP-B(2~3주): SimV7 numpy 벡터화 → 추가 5~10배(대규모 리팩토링, 회귀검증 필수). "
             "【기둥】v14.2 ML 학습 데이터 생성 속도 + v17 캠페인 MC 기반."),
            ("v11.5", "낮음", "시나리오 라이브러리",
            ...
            ("v12.1", "매우 높음", "실제 유도 알고리즘 (PNG)",
             "현재 즉시 Pk 판정 → 비례항법(PNG) 실제 물리 유도. MVP: 종말<10km만 PNG. "
             "【현실성】미사일 기동 g한계(SAM~30g·대함~10g)·탐색기 FOV 필수(무한기동 방지). "
             "【기둥】MissileObj에 heading·velocity_vec·normal_accel·max_g 추가. "
             "기존 pk_base는 PNG 회피 실패 시에만 적용 → 이중 판정. 전면 재밸런싱 동반."),
            ("v12.2", "매우 높음", "수중 음향 전파 모델",
             "단순 소나 탐지거리 → SONAR 방정식(SL−2TL+TS−(NL−DI)>DT). MVP: 수심대역 TL 룩업+수온약층 강화. "
             "【현실성】수렴지대(CZ)는 심해(>1000m)=동해만 가능, 서해(44m)는 불가 → 해역 수심 분기 필수. "
             "【기둥】_detect_range_m() 대잠 분기를 물리 계산값으로 대체."),
            ...
            ("v15.2", "높음", "다전장 연동 (공군·지상)",
             "공군 SEAD·지상 패트리엇/THAAD 동시 운용 + 합동 ROE 구역. "
             "【재배치】사실상 작전급 요소 → v18(공군 작전급)으로 흡수 권장(전술급 700초엔 반쪽). "
             "【기둥】AirDefenseZone 격자 + _select_defense_wpn() 구역 체크."),
            ("v15.3", "중간", "사이버전 모듈",
            ...
            ("v16.1", "중간", "군수·보급 모델",
             "보급함(AOE) 이동·체류 + VLS 고갈 후 귀환·재장전 + 연료 소모. "
             "【재배치】전술급 700초엔 효용 미미 → v17.2 ForcePool로 흡수 권장(작전급에서 진가). "
             "【기둥】FriendlyShipObj에 fuel_pct·ammo_resupply_t 추가."),
            ("v16.2", "높음", "지향성 에너지 무기 (레이저·HPM)",
             "HELIOS 60kW 레이저 + 고출력 마이크로파. 전력 제한 내 발사. "
             "【현실성】60kW는 드론·소형보트 한정. 초음속 탄두(현무-4급) 무력화는 MW급 필요 → 대상 제한 필수. "
             "【기둥】FriendlyShipObj에 power_budget_kw 추가. v14.4 Swarm 대응이 주용도."),
            ("v16.3", "매우 높음", "멀티플레이어 대전",
             "WebSocket 서버 + 상태동기화 + 지연보상 = 네트워크 전영역. "
             "【범위】단일 시뮬 완성도와 별개 트랙 → 보류 권장(분석 목적이면 우선순위 낮음). "
             "【기둥】v10.7 resume_tactical() 패턴 확장 재사용."),
            # ── v17.x — 작전급 시뮬레이터 ─────────────────────────────────────
