...
> **공통 규칙(보고·작업 방식·검증·Git·문서·상수 위치·Windows/Qt 앱·YAGNI)은
> 전역 `~/.claude/CLAUDE.md` 에 있다.** 여기에는 **이 프로젝트 고유의 것**만 적는다.
>
> ⚠️ **한 가지는 전역과 다르다 — GUI 검증 방식.** Argus·RL3D 는 마우스 자동화를 쓰지
> 않지만, **이 프로젝트는 무인 감사에서 pywinauto/UIA 로 버튼을 클릭한다.** 엔진 직접
> 호출(`run_v7_simulation()`)로는 GUI 워커 경로(step_cb·시그널 emit)를 안 거쳐 **exe
> 전용 버그를 놓치기 때문**이다. 그래서 전역으로 올리지 않았다(전역 10번). 아래
> '스모크 실행' 정의를 따른다.
...
| `db_terrain.py` | 지형·해협·수심 DB (GEBCO 기반). `engine_combat` 이 `STRAITS_DB` 를 import |
| `db_ocean_acoustic.py` | 해양 음향 DB — Mackenzie 음속식·Thorp 흡수·SVP·소나 탐지거리. 소나 방정식이 소비 |
| `db_ocean_environment.py` | 해양 환경 DB — 해류·조석·파고·EO/IR 감쇠 |
| `forecast_features.py` | 캠페인 즉시예측 대리모델의 피처 추출. `engine_campaign` 이 import |
| `ai_policy_infer.py` | 학습된 RL 정책 추론 진입점. `app_workers` 가 런타임에 import |
...
> **아카이브된 데이터**: `_archive/data/db_ground_threat.py`(한반도 배치 DB, 공개자료
> 415줄)는 어디서도 import 되지 않아 옮겼다(분석 F-002, 2026-09-20). 지우지 않은 이유는
> 공개 출처를 달아 수집한 실측 데이터이기 때문이고, 배선하지 않은 이유는 `engine_army` 가
> **전면 지상전을 범위 밖으로 선언**했기 때문이다(`plan_v20_army.md §8`).
...
> "기능 단위로 완성 후 커밋"·gitignore·비밀 취급은 전역 규칙 4번.
> **이 프로젝트는 커밋 메시지가 버전과 연동된다** — 다른 프로젝트와 형식이 다르다.
...
| ③ | **회귀** | 엔진 동작 무결성 + **결정론**(`sim_seed` 키 사용·신규 `random`/`numpy.random`이 RNG 순서 깨는지) + **골든 커버리지**(새 기능·새 `stats` 키가 54케이스·32지표에 실제 반영되는지 — 없으면 회귀 사각) | `python audit_verify_regression.py` 전체 PASS (FAIL이면 의도 확인 → 갱신 또는 수정) |
...
> 난이도별 작업 방식·설계 포함 항목·TaskList 운영은 전역 `~/.claude/CLAUDE.md` 2번.
> 이 프로젝트에만 해당하는 것은 아래 둘이다.

- **중간 이상 설계는 합의 후 `plan_*.md` 파일에 기록한다.** 다른 프로젝트는 합의까지지만
  여기는 파일로 남긴다 — 블록이 길어 세션을 넘기기 때문이다.
- **완결 단위마다 커밋·푸시 + `patch_queue`·`SESSION_LOG` 갱신.** 미완 Task 가 곧 재개
  지점이라 위 '세션 중단 대비' 3층과 짝을 이룬다.
...
`app_main.py`를 계층별 모듈로 분할해 두었다(mixin 6개·계층 16개, 단방향·순환 0).
아래는 그 분할에서 **실제로 당한 함정**이라 규칙으로 굳힌다 — 앞으로 모듈을 더 쪼개거나
옮길 때도 동일하게 적용한다.

1. **재할당 전역은 이름 import 금지 — 모듈 경유**. `global X`로 재대입되는 전역을
   `from mod import X` 하면 **import 시점 값이 복사**돼 재할당이 안 보인다. **py_compile·정적
   61·회귀 46×32가 전부 PASS인데 기능만 조용히 죽는다.**
...
> 일반 Qt 함정(`NoScrollComboBox` 등)과 상수 위치 규칙은 전역 `~/.claude/CLAUDE.md` 6·8번.
