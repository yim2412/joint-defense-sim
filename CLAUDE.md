...
minor : 신기능/대규모 패치 묶음의 계열 (v12.1, v12.2, 두 자리도 가능: v11.11)
seq   : 같은 minor 계열 안에서 변경 1건마다 붙는 일련번호 (.1, .2, .3 …)
```

- **changelog는 변경(추가/수정/삭제) 1건당 버전 1개**로 분리해 기록한다.
- 같은 minor 계열(기능 추가분 + 그 후속 버그수정)은 seq를 **연속**으로 잇는다.
  예: v12.1 계열 = v12.1.1~v12.1.3(PNG 기능) + v12.1.4~v12.1.6(후속 수정) / v12.2 계열 = v12.2.1~v12.2.5.
- **'patch'라는 단어는 쓰지 않는다** — `vX.Y.Z` 3단계 번호가 그 역할을 대신한다.
...
v12.2.1: [변경 내용 한 줄 요약]
```
한 작업에서 변경이 여러 건이면 changelog는 건별로 분리(v12.2.1, v12.2.2 …)하되,
커밋은 묶어서 하고 메시지 제목엔 대표 번호 범위(예: `v12.2.1~v12.2.5`)를 쓴다.
...
2. **changelog.json 갱신**: 새 항목을 배열 마지막에 추가
   ...
   ```
   `version` 필드는 변경 1건마다 `vX.Y.Z` 3단계 번호 (`v12.2.1`, `v12.2.2` 등)를 사용한다. 'patch' 표기 금지.
