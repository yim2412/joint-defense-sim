...
마이너 감사가 "변경 1건"을 보는 것과 달리, **종합 감사는 누적된 블록 전체를 통합 상태에서 한 번에 점검**한다(상호작용·하위호환·수치 안정성). 6개 영역을 모두 돌리고 **사람이 읽는 감사 보고서 파일**을 남긴다.
...
#### 2) 6개 감사 영역 (각 영역 **PASS** 또는 **해당 없음**)

| # | 영역 | 무엇을 | 방법 (에이전트 적극 활용) |
|---|------|--------|------|
| ① | **코드·로직** | 블록 전체 누적 diff의 상호작용·하위호환 버그 (플래그 OFF 시 기존 결과 동일 포함) | `/code-review high` (블록 범위 diff) |
...
| ④ | **통합 MC** | 기준 시나리오 전체 회귀 MC의 수치 안정성 | 기준값 메모리(`project-baseline-*`)와 대조 |
| ⑤ | **exe·빌드** | 전체 빌드 성공 + **번들 무결성**(데이터 파일 포함 확인) + 스모크 | 빌드 + 사용자 스모크 ([[feedback-smoke-run]]) |
| ⑥ | **위생** | changelog·`_PLANS` 코드명/완료항목 잔류, 헤더·`APP_VERSION`·changelog 정합 | **Grep 자동 스캔** ([[feedback-plans-changelog-hygiene]]) |
...
| 영역 | 판정 | 발견 항목 | 조치 |
|------|------|-----------|------|
| ① 코드·로직 | PASS / 발견N | ... | vXX.YY.ZZ에서 수정 |
| ② DB·수치  | PASS/해당없음 | ... | ... |
| ③ 회귀     | PASS | — | — |
| ④ 통합 MC  | PASS | 기준값 대조: 요격률 NN%±N (기준 NN%) | — |
| ⑤ exe·빌드 | PASS | 번들 무결성 OK·스모크 OK | — |
| ⑥ 위생     | PASS/발견N | ... | ... |
...
**6영역 전부 PASS 또는 '해당 없음'**이어야 블록 종료를 선언한다. 미통과 영역이 있으면 수정·재감사 후 다시 판정. 통과 시 보고서 종합 판정에 '통과' 기록 + 커밋·푸시.
