...
major : 아키텍처 전환 (v7 = PyQt6 + 시간스텝 엔진). 그대로 표기(패딩 안 함)
minor : 신기능/대규모 패치 묶음의 계열 (v12.01, v12.02, 두 자리도: v11.11)
...
- 같은 minor 계열(기능 추가분 + 그 후속 버그수정)은 seq를 **연속**으로 잇는다.
  예: v12.1 계열 = v12.01.01~v12.01.03(PNG 기능) + v12.01.04~v12.01.06(후속 수정) / v12.2 계열 = v12.02.01~v12.02.05.
...
v12.02.01: [변경 내용 한 줄 요약]
```
한 작업에서 변경이 여러 건이면 changelog는 건별로 분리(v12.02.01, v12.02.02 …)하되,
커밋은 묶어서 하고 메시지 제목엔 대표 번호 범위(예: `v12.02.01~v12.02.05`)를 쓴다.
...
2. **changelog.json 갱신**: 새 항목을 배열 마지막에 추가
   ...
   ```
   `version` 필드는 변경 1건마다 `vX.YY.ZZ` 3단계 번호 (`v12.02.01`, `v12.02.02` 등, minor·seq 두 자리)를 사용한다. 'patch' 표기 금지.
