...
마이너 감사가 "변경 1건"을 보는 것과 달리, **종합 감사는 누적된 블록 전체를 통합 상태에서 한 번에 점검**한다(상호작용·하위호환·수치 안정성). 7개 영역을 모두 돌리고 **사람이 읽는 감사 보고서 파일**을 남긴다.
...
#### 2) 7개 감사 영역 (각 영역 **PASS** 또는 **해당 없음**)

| # | 영역 | 무엇을 | 방법 (에이전트 적극 활용) |
|---|------|--------|------|
| ① | **코드·로직** | 블록 전체 누적 diff의 상호작용·하위호환 버그 (플래그 OFF 시 기존 결과 동일 포함) + **신기능 체크리스트 8항목 블록 재검증**(아래) + **전역 상태 오염**(전역 DB `ENEMY_DB`/`FRIENDLY_DB`/`SHIP_DB`를 `.copy()` 없이 참조 후 mutate / `cfg = dict(cfg)` 미복사 → MC 반복 간 조용한 오염) | `/code-review high` (블록 범위 diff) + Grep 스캔 |
| ② | **DB·수치** | 블록에서 바뀐 DB 값을 공개 제원·교리와 대조 | **Explore 에이전트 팬아웃** (또는 수동) |
| ③ | **회귀** | 엔진 동작 무결성 | `python verify_regression.py` 전체 PASS (FAIL이면 의도 확인 → 골든 갱신 또는 버그 수정) |
| ④ | **통합 MC + 성능** | 기준 시나리오 전체 회귀 MC의 수치 안정성 + **wall-time 회귀 가드**(기준 단발 1회·전장 1회 실행시간을 이전 블록 대비, 급증 1.5배+면 원인 규명) | 기준값 메모리(`project-baseline-*`)와 대조 + 시간 측정·보고서 기록 |
| ⑤ | **exe·빌드** | 전체 빌드 성공 + **번들 무결성**(데이터 파일 포함 확인) + 스모크 + **MC 중단(abort) 후 워커 잔존·풀 정리**(알려진 이슈: 중단 시 제출된 배치 백그라운드 잔존) | 빌드 + 사용자 스모크 ([[feedback-smoke-run]]) + 중단 1회 테스트 |
| ⑥ | **위생** | changelog·`_PLANS` 코드명/완료항목 잔류, 헤더·`APP_VERSION`·changelog 정합, **죽은 코드(호출처 0 메서드/위젯)**, **상수·임계값 교차 정합**(블록에서 바뀐 상수가 서로 모순 없는지 — 예: `MAX_SIM_TIME` vs `BATTLE_HORIZON_S`), **보안·문서**(개인키 미커밋·`dist`/`build` gitignore·CLAUDE.md의 engine_v7 함수표가 실제 코드와 일치) | **Grep 자동 스캔** ([[feedback-plans-changelog-hygiene]]) |
| ⑦ | **하위호환** | 저장된 **구버전 시나리오 cfg**(과거 `enable_xxx` 누락 dict)로 로드·실행 시 정상 동작 | 구버전 cfg dict로 `run_v7_simulation`/`run_battle_simulation` 1회 호출 |

> 사람은 에이전트 결과를 **검토·승인**한다. 영역이 그 블록과 무관하면(예: UI만 바뀐 블록의 ②DB) '해당 없음'으로 명시 후 생략.
...
#### 4) 산출물 — 감사 보고서 파일 (`감사보고서.md`, repo 루트)
...
**7영역 전부 PASS 또는 '해당 없음'**이어야 블록 종료를 선언한다. 미통과 영역이 있으면 수정·재감사 후 다시 판정. 통과 시 보고서 종합 판정에 '통과' 기록 + 커밋·푸시.
