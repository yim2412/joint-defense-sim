...
║  미구현-1~5 수정 (플랫폼 종말회피 / current_depth / MC샘플로그)             ║
...
# ════════════════════════════════════════════════════════════════════════════
#  [코드 전체 기능 목록] — v6.1 최신화
#
# ── 기반 엔진 [기능 01~18] ────────────────────────────────────────────────────
#  [01] 이벤트 큐(heapq) 기반 교전 시뮬레이션
#  [02] 몬테카를로 1000회 (Beta분포 Pk, save_nth MC샘플로그)
#  [03] 날씨 3종 시나리오 비교 (각 300회)
#  [04] REQ-01~08 요구조건 자동 판정 (RTM)
#  [05] TEWA 자동 무기할당 (HGV/QBM/어뢰 특수처리)
#  [06] 교전 비용 자동 집계
#  [07] Jane's 전술 교전도 (거리-고도/수심 2D, 베지어 요격선)
#  [08] 엑셀 보고서 9시트 자동생성 (Sheet9 MC샘플로그)
#  [09] 콘솔 교전로그/RTM/상황서술 출력
#  [10] 적 발사 미사일·어뢰 별도 ThreatEvent 처리
#  [11] ShipStatus: 피격 즉시 교전 중단, 헬기·P-3C 통계 포함
#  [12] Shoot-Shoot-Look 교전교리
#  [13] 운동학적 Pk 동적 보정
#  [14] DF-17 HGV 특수로직 (SM-3만 교전)
#  [15] KN-23 QBM 특수로직 (SM-6 우선)
#  [16] max_cd 무기 사거리 반영 계산
#  [17] 날씨 8종 지원
#  [18] ENEMY_DB 32종 + normalize_enemy_db() 자동 전처리
#
# ── v5.14 (NEW-A~E) [기능 19~23] ────────────────────────────────────────────
#  [19] NEW-A: 개별 미사일 Pk 판정
#  [20] NEW-B: 음향 기만기 AN/SLQ-25 + 함정 회피 기동
#  [21] NEW-C: 적 다발 발사 (RANDOM/FIXED/MAX)
#  [22] NEW-D: 미사일 종말 회피 기동 (20km 이내)
#  [23] NEW-E: 적 회피 기동 (속도증가·고도수심변경·재추적)
#
# ── v6.8.1 (NEW-F, BUG-1~3) [기능 24~29] ────────────────────────────────────
#  [24] NEW-F 간단: self_defense_pk 채프·플레어 Pk 감소
#  [25] NEW-F 중간: enemy_ciws_pk 적 CIWS 요격
#  [26] BUG-1: 수상함·잠수함 미사일 발사 보장
#  [27] BUG-2: 잠수함 수심 변경 altitude_m 반영
#  [28] BUG-3: ENEMY_DB 전역 상태 오염 방지
#  [29] 한글 폰트 직접 등록 + REQ 기준무기 자동탐색
#
# ── v5.16.1 (NEW-G·H, BUG-4~6, 미구현-1~5) [기능 30~38] ────────────────────
#  [30] NEW-G: ECM 재밍 (거리 반비례, 50km 기준, enable_ecm)
#  [31] NEW-H: 함재 헬기 대잠 운용 (AW-159/MH-60R, HeloEvent 클래스)
#             비행갑판 출격, 날씨 풍랑↑ 불가
#  [32] BUG-4: normalize_enemy_db() 플랫폼 종말회피 자동 설정
#  [33] BUG-5: missile_salvo_fixed 범위 초과 경고
#  [34] BUG-6: monte_carlo 독립 ShipStatus 집계 (mc_hit_avg)
#  [35] 미구현-1: enable_missile_evasion 플랫폼 자체 적용
#  [36] 미구현-2: current_depth 잠수함 수심 그래프 반영
#  [37] 미구현-5: save_nth MC 샘플 로그 Sheet9 저장
#  [38] plot_all return_fig=True 파라미터 (대시보드 연동용)
#
# ── v6.1 (NEW-I, 대시보드) [기능 39~41] ────────────────────────────────────
#  [39] ★ NEW-I: P-3C 오라이온 해상초계기 대잠 운용
#             실제 대한민국 해군 운용 자산 기준
#             포항기지 출격 (육상기지 base_type='land')
#             준비 40분 + 기지→작전해역(300km) + 목표까지 비행시간
#             Mk.46 경어뢰 4발 탑재
#             풍랑·폭풍에서도 출격 가능 (태풍만 불가)
#             헬기 사거리 밖 잠수함 공격 가능
#             enable_p3c ON/OFF
#
#  [40] ★ 실시간 Streamlit 대시보드 (dashboard_v6_2.py)
#             파라미터 사이드바 실시간 조작
#             탭: 전술교전도 / RTM / 교전로그 / MC통계
#             XLSX 다운로드 버튼
#             streamlit run Dashboard_v6_8_4.py 로 실행
#
#  [41] ★ 항공기별 날씨 제한 (weather_limits 필드)
#             FRIENDLY_AIRCRAFT_DB 각 항목에 weather_limits 딕셔너리
#             헬기: 풍랑↑ 불가 / P-3C·P-8A: 태풍만 불가
#
# ── v6.2 (NEW-J, BUG-7/8, DEAD-1~6) [기능 42~45] ────────────────────────
#  [42] ★ NEW-J: P-8A 포세이돈 해상초계기 대잠 운용
#             P-3C 후속 기종 (Boeing 737-800ERX 기반)
#             포항기지 출격, 준비 30분 (P-3C 40분보다 빠름)
#             Mk.46 경어뢰 5발 탑재 (P-3C 4발보다 1발 많음)
#             소노부이 탐지 보너스 +18km (P-3C +15km보다 우수)
#             태풍 외 전천후 운용 (P-3C와 동일)
#             enable_p8a ON/OFF
#
#  [43] ★ BUG-7 수정: HeloEvent.uid 고정값("HELO") → 클래스 카운터 기반 동적 ID
#             다수 헬기/P-3C/P-8A 동시 운용 시 uid 중복 해소
#
#  [44] ★ BUG-8 수정: __main__ inventory에 Mk.46 경어뢰 누락 추가
#             독립 실행 시 P-8A/P-3C 어뢰 재고 0 문제 해결
#
#  [45] ★ DEAD-1~6 데드코드 정리
#
# ── v6.2.1 패치 (BUG-9, DEAD-7, MC수정) ─────────────────────────────────────
#  [46] ★ BUG-9 수정: HeloEvent._id_counter 리셋 누락
#             run_single_sim 진입 시 ThreatEvent와 동일하게 HeloEvent도 리셋
#             MC 1000회 실행 시 H번호 무한 누적 문제 해결
#  [47] ★ DEAD-7 제거: HeloEvent.enemy_self_def_apps 잔존 필드 삭제
#             DEAD-5/6 정리 시 누락된 HeloEvent 내 미사용 필드 제거
#  [48] ★ MC 항공기 효과 반영 수정
#             monte_carlo silent=True 시에도 _try_aircraft 실행
#             P-8A/P-3C/헬기 출격이 MC 1000회 통계에 정상 반영됨
#
# ── v6.3 (NEW-K) [기능 49~52] ─────────────────────────────────────────────
#  [49] ★ NEW-K: SHIP_DB — 함정 종류별 스펙 DB
#             KDX-III (이지스 구축함): 24채널, 센서 1200km, SM-3/SM-6/SM-2 보유
#             KDX-II  (구축함)       : 12채널, 센서 200km, SM-2/RAM 보유
#             FFX     (호위함)       : 8채널, 센서 150km, SM-2/RAM 보유
#
#  [50] ★ NEW-K: FLEET_PRESETS — 한국 해군 교리 기반 편대 프리셋 5종
#             단독 작전 / 기동전단 기본 / BMD 중점 / 대잠 중점 / 최대 편대
#
#  [51] ★ NEW-K: Ship / Fleet / FleetStatus / FleetChannelManager 클래스
#             Ship: 개별 함정 (자체 재고·채널·조명기·상태)
#             Fleet: 편대 관리 + 편대 TEWA (위협별 최적 함정 자동 배분)
#             FleetStatus: ShipStatus 호환 인터페이스 (Dashboard 무수정 연동)
#             FleetChannelManager: 전 함정 채널 합산 (Gantt 표시용)
#
#  [52] ★ NEW-K: build_fleet() / run_fleet_sim_core()
#             enable_fleet=True 시 기존 run_single_sim 대신 편대 시뮬 실행
#             enable_fleet=False 시 기존 엔진 100% 그대로 동작 (하위 호환)
...
#
#  [55] ★ NEW-L: generate_random_enemy_fleet() — 랜덤 적군 편대 생성기
#             난이도별 pool에서 무작위 종류·수량 생성
#
#  [56] ★ NEW-L: build_enemy_threat_list() — 적군 편대 위협 목록 통합 생성
#             single/preset/custom/random 4개 모드 지원
#             run_single_sim + run_fleet_sim_core 공통 사용
#
#  [57] ★ NEW-L: 아군 편대 커스텀 모드
#             build_fleet() fleet_mode='custom' 지원
#             fleet_custom_ships 리스트로 함정 종류·이름 직접 지정
#
# ── v6.6 (NEW-N) [기능 58~60] ─────────────────────────────────────────────
#  [58] ★ NEW-N: 다층 방어 (Layered Defense) — 한국 해군 CEC 교리 기반
#             1차 교전 실패 시 다음 레이어 함정이 자동 인계
#             KDX-III → KDX-II → FFX 순서 (역할별 사거리 기반)
#             enable_fleet=True 시 자동 활성화
#
#  [59] ★ NEW-N: run_comparison() — A vs B 시나리오 비교 함수
#             두 시나리오를 각각 실행하고 핵심 지표 비교 반환
#             요격률·MC성공률·비용·REQ 충족 비교
#
#  [60] ★ NEW-N: Dashboard A vs B 비교 모드
#
# ── v6.6.1 패치 ─────────────────────────────────────────────────────────────
# · BUG-1: 편대 모드 무기 재고 사용량 음수 → 편대 합산 초기값으로 수정
# · BUG-2: 내부 출력 버전 v6.8.1 → v6.6.1 수정
# · BUG-3: 랜덤 편대 항상 1종 → 난이도별 최소 종류 보장
# · BUG-4: TEWA 부하 분산 미작동 → assigned_count 기반 균등 배분
# · BUG-5: 수상함/잠수함 REQ-02 C&D 0초 → CIWS 최소 거리 기준으로 수정
#
# ── v6.7 패치 ─────────────────────────────────────────────────────────────────
# · NEW: 상세 분석 탭 추가 (📈)
# · ① 편대 함정별 생존 현황 시각화
# · ② MC 통계 강화 (요격률 바차트·비용 박스플롯)
# · ③ 교전 타임라인 개선 (함정별 시퀀스)
# · ④ 비용 분석 차트 (무기별 파이·위협당 평균)
# · ⑤ 위협 카테고리별 요격률 바차트
#
# ── v6.8 패치 ─────────────────────────────────────────────────────────────────
# · 교리①: 다방위 공격 — enable_multibearing + assign_bearings() + Top-down 뷰
# · 교리②: SM-3 미드코스 단계 제한 (alt>=40km 조건 추가)
# · 교리③: ECM 취약도 차등 — radar/ir/combined/none 유형별 효과 차등 적용
# · CEC: 사전 동시 배정 방식 추가 — enable_cec_preassign ON/OFF
# · 코드 정리: dead code 재점검, normalize_enemy_db 확장
#
# ── v6.8.1 패치 ───────────────────────────────────────────────────────────────
# · 프로젝트명 변경: 이지스 기동전단 통합 방어 시뮬레이터
# · 폰트 설정 확인 (Malgun Gothic 이미 적용됨)
# · 기존 구버전 잔존 문자열 전체 정리 완료
# · 전술교전도 타이틀 프로젝트명 반영
# · 엑셀 보고서 파일명 프로젝트명 반영
# · Dashboard 상세 분석 탭 개편 (기동전단 관점)
#
# ── v6.8.2 패치 ───────────────────────────────────────────────────────────────
# · 헤더 v6.7 잔존 → v6.8.2 수정
# · ax.set_title 간소화 — 프로젝트명/버전 제거, 범례만 표시
# · fig.suptitle v6.8 → v6.8.2, 1줄로 간결화
# · subplots_adjust top 0.94→0.93 으로 여백 확보
#
# ── v6.8.3 패치 ───────────────────────────────────────────────────────────────
...
# · MH-60R 시호크 Dashboard 드롭다운에서 제거 (한국 해군 미운용 기체)
#             사이드바 토글로 활성화
#             시나리오 B 핵심 파라미터 별도 설정
#             비교 탭: 지표 카드·바차트·시나리오별 교전 로그
#             LAYERS 전역 상수 제거 (TACTICAL/SURFACE/SUB_LAYERS로 대체됨)
#             HELO_WEATHER_LIMIT 제거 (_HELO_WX 중복)
#             monte_carlo/scenario_comparison dm_eff_base 미사용 파라미터 제거
#             pk_mean/pk_std 4단계 데드 파이프라인 전체 제거
#             ShipStatus.enemy_self_def_total / friendly_pk_reduced 연쇄 데드필드 제거
#
# ════════════════════════════════════════════════════════════════════════════
#
#  [향후 계획 — v6.x 우선순위]
#
#  구현 완료: ECM(NEW-G), 헬기(NEW-H), P-3C(NEW-I), P-8A(NEW-J), 대시보드(v6.2)
#
#  ┌───┬────────────────────────────┬──────┬─────────────────────────────┐
#  │순위│ 계획                       │난이도│ 이유                        │
#  ├───┼────────────────────────────┼──────┼─────────────────────────────┤
#  │ 1 │ 완전 양방향 교전 엔진      │매우높│ 적도 아군 미사일 요격       │
#  │   │ (v7.0)                     │      │ 엔진 전면 재설계            │
#  ├───┼────────────────────────────┼──────┼─────────────────────────────┤
#  │ 4 │ PyQt6 네이티브 UI 전환     │매우높│ Streamlit 한계 극복         │
#  │   │ (v7.0) ★ UI 재설계        │      │ 실시간 애니메이션, 인터랙션 │
#  │   │ · 런처 + 자동 업데이트     │      │ 지도 클릭, 창 자유 배치     │
#  │   │ · CPU/RAM/GPU 모니터링     │      │ 경과시간·MC진행률 실시간    │
#  ├───┼────────────────────────────┼──────┼─────────────────────────────┤
#  │ 5 │ 지형·해상 환경 모델        │ 높음 │ DEM/수온층 데이터 연동      │
#  │   │ (v7.x)                     │      │ 외부 의존성 높음            │
#  └───┴────────────────────────────┴──────┴─────────────────────────────┘
#
...
#
# ── 고증 수치 수정 ────────────────────────────────────────────────────────────
# · BUG-1: KDX-II 대공 탐지거리 200→120 km (MW-08 레이더 실제 탐지 한계)
# · BUG-2: FFX 대공 탐지거리 150→100 km (MW-08/SPS-550K 실제 탐지 한계)

#  ★ UI 전환 전략 (v6.x → v7.0)
#    v6.x : Streamlit 유지, 엔진 개발에 집중
#    v7.0 : 양방향 교전 엔진 완성 시점에 PyQt6 UI 병행 개발 시작
#    v7.0 : PyQt6 런처로 완전 전환 (엔진 코드는 그대로 재사용)
#    이유 : 기능이 완성된 시점에 UI 설계해야 누락·재작업 최소화
#
# ════════════════════════════════════════════════════════════════════════════
#  필수: pip install matplotlib numpy scipy openpyxl pillow
#  대시보드: pip install streamlit pandas → streamlit run dashboard_v6_1.py
# ════════════════════════════════════════════════════════════════════════════