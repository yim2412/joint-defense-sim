...
> **현재 단계:** 해군 — 이지스 기동전단 통합 방어 + 공군·지상 작전급(연안방공·상륙·도미노) + 한국형 미사일방어 5계층 + 육해공 합동 화력(협조 타격) + 합동작전 통합 보고서(군별 기여도) + 합동작전 사령부(자원 충돌 경고) (v21.12)
...
| 🪖 **육군 (Land)** | ✅ 작전급 구현 | 연안 방공망(5계층 요격) · 해상 상륙작전(교두보 확보) · 방공망 제압 도미노 |
...
- **탄도탄 방어(BMD) 5계층** *(선택 옵션 — 연안·본토 방어 시나리오에서 켜서 씁니다)* — 이지스 어쇼어 SM-3(중간단계) · THAAD(종말 고고도) · L-SAM(종말 상층) · 패트리엇 PAC-3 MSE(종말 중층) · 천궁-II(종말 하층 점방어). 탄도탄 종말 강하 궤적에 따라 접근 거리대별 순차 교전
...
| `_audit_ui_shot.py` | UI 시각 감사 — 화면 점유 없이 표시 깨짐(&&·잘림·작은 글씨) 검사·캡처 (감사용) |
...
> **Current stage:** Navy — Aegis task force integrated defense + air-force & ground operational layers + 5-layer Korean missile defense + joint fires (coordinated strike) + joint operations report (per-service contribution) + joint chiefs of staff (resource conflict alerts) (v21.12)
