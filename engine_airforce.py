...
#   대당 선형이고 **상한이 없다**(4대면 ×1.6, AEW ×1.3와 곱해 ×2.08). 감사는 "상한·체감률이
#   필요하지 않은가"를 지적했으나, **상한값도 체감률도 공개 교리에 근거가 없다** — min(n,2)든
#   0.08이든 지어낸 숫자가 된다. 근거 없는 수식을 넣는 것은 모델 부채이고, 실측 제공권 이동도
#   0.58→0.69로 과격하지 않아 눈에 띄는 왜곡이 없다. 그래서 **선형·무상한을 유지한다.**
#   (다음 감사가 이 항목을 또 발견해 또 고민하지 않도록 여기에 못 박는다. 고치려면 먼저
#    급유 소요·체공 연장의 공개 교리 수치를 확보할 것.)
...
# v19.5 CAS(근접 항공 지원) — 제공권과 **직교하는 레버**. 제공권은 교전 win_p·score(성패)에
# 작용(v19.2)하지만, CAS는 아군 함정 피해(dmg) 경감에 작용(_apply_engagement). 근접 엄호=이기든
# 지든 손실을 줄임. 같은 축(win_p)에 또 붙이면 제공권과 중복이므로 의도적으로 다른 레버 선택.
_CAS_RELIEF_PER_EFFORT = 0.10   # CAS 소티율 1당 zone 교전 dmg 경감(보수적)
_CAS_RELIEF_CAP        = 0.50   # 경감 상한 — 근접 지원도 손실을 절반까진 못 없앰(보수적)
_CAS_REQ_THRESHOLD     = 1.0    # 지원 요청 우선도 임계 — (1-통제도)×적규모가 이 이상인 zone만 CAS 요청
_CAS_MAX_PER_ZONE      = 2      # 요청 zone당 CAS 차출 상한(대) — 전 기체가 CAS로 빠져 CAP 붕괴 방지
...
# ── 공군 편성 프리셋 (기본 패키지) ─────────────────────────────────────────────
AIR_FORCE_PRESETS = {
    # v20.1: KC-330 시그너스 편입 — 공군은 2018~19년 4대를 도입해 운용 중이다.
    #   급유기는 전투기 체공 시간을 늘려 제공권 전력을 끌어올린다(직접 교전 없음).
    '한국 공군 단독': [
    ...
    '한미 연합 공군 패키지': [
        ...
        ('E-737 피스아이', 1), ('RQ-4 글로벌호크', 2), ('KC-330 시그너스', 4),
    ],
    '최소 방공 (제공권 열세)': [
...
class AirCampaign:
    ...
    def __init__(self, cfg: dict, bases: list | None = None,
        ...
        **위임**한다(이중 계상 방지). 소티 집계·표적 재건은 그대로 여기서 한다."""
        ...
        # v19.5 CAS — 근접 항공 지원(요청 기반, dmg 경감). support_req 없으면 미발동(하위호환).
        self._cas_acc   = 0.0   # 누적 CAS 소티
        self._n_cas_req = 0     # 해군이 근접지원 요청한 zone-틱 누적(요청 임계 초과)
        self._tl_cap    = []    # 틱별 CAP 배정 기체 수(임무 타임라인 v19.5.2)
        self._tl_sead   = []    # 틱별 SEAD 배정 기체 수
        self._tl_strike = []    # 틱별 전략폭격 배정 기체 수
        self._tl_cas    = []    # 틱별 CAS 배정 기체 수
        self._cas_log   = []    # CAS 요청·대응 로그 [{'t','zone','relief'}] (t는 틱 인덱스=시간h)
        self._tick_i    = 0     # 틱 카운터(로그 t_h용 — 캠페인 t_h와 동기)
    ...
    def tick(self, zone_threat: dict, support_req: dict | None = None,
        ...
        # 1) 임무 배정(CAP를 고위협 SLOC zone에, SEAD를 방공망 활성 zone에, CAS를 요청 zone에)
        self._assign(zone_threat, req)
        ...
        for u in self.units:
            if u.mission == 'CAP' and u.zone in zone_power:
                ...
                self._cas_acc += u.sortie_rate / 24.0
                n_cas += 1
        self._tl_cap.append(n_cap); self._tl_sead.append(n_sead)
        self._tl_strike.append(n_strike); self._tl_cas.append(n_cas)
        # v19.5: 이번 틱 CAS 요청·대응 로그(요청 있었고 실제 CAS 배정된 zone만 relief 기록)
        if n_cas > 0:
            for z, relief in self.cas_support().items():
                if relief > 0:
                    self._cas_log.append({'t': self._tick_i, 'zone': z, 'relief': round(relief, 3)})
    ...
    def _assign(self, zone_threat: dict, support_req: dict | None = None):
        """임무 배정 — 방공망 활성 zone엔 SEAD기 1대씩 먼저(CAP에서 차출), v19.5 해군 지원요청
        임계 초과 zone엔 CAS기 차출(요청 큰 순·zone당 상한), 나머지 CAP기는 고위협 SLOC zone
        우선(라운드로빈), 정찰기는 ISR, 전략폭격기는 기지 타격, 조기경보기는 전역 통제(승수)."""
        req = support_req or {}
        ...
        # v19.5: 해군 지원요청 임계 초과 zone에 CAS 차출(요청 큰 순, zone당 _CAS_MAX_PER_ZONE 대)
        cas_todo = []
        cas_req_zones = [z for z in SLOC_ZONES if req.get(z, 0.0) >= _CAS_REQ_THRESHOLD]
        self._n_cas_req += len(cas_req_zones)   # 요청 자체 계측(배정 성공과 별개)
        for z in sorted(cas_req_zones, key=lambda z: req.get(z, 0.0), reverse=True):
            cas_todo.extend([z] * _CAS_MAX_PER_ZONE)
        ci = 0
        for u in self.units:
            ms = u.missions
            if u.role == 'isr' and 'recon' in ms:
                ...
                u.zone = cas_todo.pop(0); u.mission = 'CAS'     # 요청 zone에 근접지원 차출
            elif 'CAP' in ms:
    ...
    def cas_support(self) -> dict:
        """v19.5: zone별 CAS 피해경감 계수 {zone: relief 0~1}. CAS 소티율 합×보수계수, 상한
        _CAS_RELIEF_CAP. 캠페인 _tick_engagements가 조회해 그 zone 교전 dmg를 (1-relief)배 경감.
        CAS 미배정(요청 없음·기체 없음)이면 전부 0 → dmg 불변(v19.4 bit-identical)."""
        out = {z: 0.0 for z in SLOC_ZONES}
        for u in self.units:
            if u.mission == 'CAS' and u.zone in out:
                out[u.zone] += u.sortie_rate
        for z in out:
            out[z] = min(_CAS_RELIEF_CAP, _CAS_RELIEF_PER_EFFORT * out[z])
        return out
    ...
    def summary(self) -> dict:
        ...
        out = {
            ...
            'n_cas_requests':       self._n_cas_req,          # v19.5 해군 지원요청 zone-틱 수
            'air_timeline':         {'superiority': self._tl_sup, 'cap': self._tl_cap,
                                     'sead': self._tl_sead, 'strike': self._tl_strike,
                                     'cas': self._tl_cas, 'cas_log': self._cas_log},
        }
