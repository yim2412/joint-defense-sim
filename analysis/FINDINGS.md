...
- 위치: engine_combat.py:5720 `_n_tot = self.stats['total_threats'] + self.stats['suicide_threats']`  (F-009 수정으로 +11 이동)
- 근거: [실측]
- 이력: [신규]
- 심각도: 높음
...
### F-007 · 축: 뼈대 · 상태: 수정됨
- 위치: engine_combat.py:6472 `def _apply_ship_evasion(self, evade_r_base: float | None = None,`  (수정 후 — 부모 계약 복원된 시그니처)
- 근거: [코드]
- 이력: [신규]
- 심각도: 중간
...
- 위치: engine_combat.py:5658 `def _pk_of(self, wpn_info: dict) -> float:`  (수정 후 — 4개 소비처 통합 헬퍼)
- 근거: [실측]
- 이력: [신규]
- 심각도: 중간
...
### F-004 · 축: 뼈대 · 상태: 오탐
- 위치: engine_combat.py:6806 `def _compile(self) -> dict:`  (BattleEngine 쪽)
- 근거: [실측]
- 이력: [신규]
- 심각도: 중간
