...
_FEATURES = [
    ...
    ("⚡  시뮬레이션 모드 선택",
     "빠름(200회) / 표준(1,000회) / 정밀(10,000회) 중 목적에 맞게 선택. "
...
class SplashWindow(QWidget):
    ...
    def closeEvent(self, event):
        # 창 위치·크기 저장 (복원은 __init__의 _apply_window_geometry가 담당)
        try:
    ...
    def _build_home_page(self) -> QWidget:
        ...
        # ── 상단(하늘 영역): 타이틀, 좌측 정렬 ──
        # 엠블럼은 싣지 않는다 — 같은 화면 왼쪽 사이드바 머리에 이미 같은 엠블럼과
        # 앱 이름이 있어 로고가 두 번 보였다(v21.07.16).
    ...
    def _build_help_tab(self) -> QWidget:
        ...
        _STEPS = [
            ...
            ("4", "반복 횟수 설정",   "몬테카를로 반복 횟수를 설정합니다. (빠름 200 / 표준 1,000 / 정밀 10,000)"),
        ...
        _FAQ = [
            ("몬테카를로 횟수를 얼마로 설정해야 하나요?",
             "편성끼리 비교할 때는 표준(1,000회)이면 충분합니다 — 요격률 표준편차가 이미 수렴합니다.\n"
             "빠름(200회)은 설정을 바꿔가며 훑을 때, 정밀(10,000회)은 최종 보고용입니다."),
            ("Pk가 0으로 나옵니다. 왜인가요?",
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        # 이 탭은 사용자에게 보이는 '앞으로 할 일'만 싣는다. 개발 절차(감사·리뷰 규칙)와
        # 판정이 끝난 항목은 싣지 않는다. 계획 정본은 개발 문서(plan_roadmap.md)에 있다.
        _PLANS = [
            # ── 분석 지표 — 편성 비교의 기준을 바로잡는다 ─────────────────────
            ("지표", "중간", "편성 비교 기준 교체 (요격률 → 교환비·방어 포화도)",
             "요격률은 편대를 키울수록 떨어진다 — 큰 편대가 오래 버티는 동안 적이 더 많이 쏘기 때문에 "
             "비교의 분모가 편성마다 달라진다(실측: 1척 21.7% > 6척 16.1% > 7척 12.5%). "
             "적정 편대 추천은 생존율·교환비 기준으로 이미 교체했고, 남은 것은 요구조건 판정·"
             "요격률 게이지·결과 등급 배너다. 지금은 첫 화면이 '함정 수를 보강하라'는 처방을 내는데 "
             "실제로 함정을 늘리면 그 수치는 떨어진다. 교환비·요격당 비용·방어 포화도·탄약 효율은 "
             "이미 계산되고 있으므로 자리를 바꾸는 작업이다."),
            ("지표", "낮음", "요구조건 임계값을 분석자가 정하도록",
             "전술 요구조건 7항목의 합격 기준이 코드에 고정돼 있다. 요구조건은 본래 분석자가 "
             "정의하는 입력이므로 화면에서 설정할 수 있어야 한다. 아울러 응답시간·주요 SAM 잔여 "
             "두 항목은 현재 어떤 편성으로도 전부 통과해 변별력이 없다 — 기준 재설정이 필요하다."),
            # ── 기본 설정 — 21개 메이저를 거치며 쌓인 기본값을 현재 모델로 ────
            ("기본값", "중간", "기본 설정을 현재 교전 모델로 갱신",
             "하위 호환을 위해 신규 기능을 모두 기본 꺼짐으로 도입한 결과, 기본 설정이 초기 버전의 "
             "교전 모델에 가깝다. 옵션 54개 중 24개가 아직 '실험적'이고, 한국형 미사일방어 5계층은 "
             "전부 기본 꺼짐이다. 검증이 끝난 기능은 실험적 표시를 떼고, 항상 존재하는 물리(비례항법 "
             "종말 유도 등)는 기본 적용으로 승격한다. 비례항법을 켜면 격추 거리 표시가 일부에서 "
             "전체로 살아난다."),
            ("기본값", "낮음", "시나리오 난이도 표기 정정",
             "'입체 포화 (최강)' 프리셋이 실제로는 위협 38발로, 랴오닝 항모전단(181발)보다 훨씬 약하다. "
             "정밀 위협을 1발씩만 편성해 실제 교리의 다축 동시 포화와 어긋난다. 편성을 현실화하고 "
             "난이도 표기를 실제 발생 위협량에 맞춘다."),
            # ── 3D 전장 표현 ──────────────────────────────────────────────────
            ("3D", "낮음", "3D 전장 표현 개선 (요격·피격 구분·범례)",
             "현재 요격 성공과 아군 함정 피격이 같은 노란 점으로 표시돼 무엇이 일어났는지 구분되지 않는다. "
             "미사일과 명중 지점에 이름표가 없고 범례도 없으며, 명중 표시가 2.5초만 남는다. "
             "아군 미사일의 종말 지점은 아예 표시되지 않는다."),
            # ── 전력 DB ───────────────────────────────────────────────────────
            ("전력 DB", "중간", "육해공 전력 DB 심화 확충",
             "해군은 개별 함정·무기 제원까지 내려가 있으나 공군·육군은 전력 지수 하나로 추상돼 있어 "
             "층별 충실도가 고르지 않다. 적 공군 기종(J-20·J-16·Su-35 등)과 공중급유기·전자전기·"
             "무인기를 추가해 제공권 모델을 정교화한다. 실측 교전에서 효과가 발현하는 항목 위주로 "
             "확충한다 — 규모만 늘리면 중복 시나리오만 늘어난다."),
            # ── 대잠전 ────────────────────────────────────────────────────────
            ("대잠전", "중간", "대잠전 균형 (능동 소나 역탐지 딜레마)",
             "능동 소나를 켜면 잠수함에 들킨다는 딜레마 자체는 성립시켰다 — 접촉이 끊긴 시간만큼 "
             "수색 구역이 넓어져 잠수함의 도주가 실제 생존으로 이어진다. "
             "【남은 과제】효과의 절대 크기가 작다. 위협 잠수함이 원거리에서 발사하고 이탈해 "
             "애초에 아군 피해 규모가 작기 때문이다. 잠수함의 교전 방식(접근·재공격·매복)을 "
             "손대야 대잠전이 실제 균형을 갖는다."),
        ]
