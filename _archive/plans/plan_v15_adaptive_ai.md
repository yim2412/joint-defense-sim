...
| v15.01.03 | ✅ 완료 (2026-06-12) — **슬라이딩 윈도우**(누적→최근 20초 구간 요격률, 관성 제거) + **한 칸씩 점진 전환**(포화→분산→기만 단계를 밟음, 급점프 방지, 양방향) + 교전 로그 ⚡ 강조 + 전환 횟수 집계(`enemy_tactic_switches`). 검증: 40s 포화→분산→60s 분산→기만 점진 확인. **난이도별 민감도는 제외**(랜덤 적 모드 전용 값이라 프리셋 위주 사용에 효과 제한적). |
