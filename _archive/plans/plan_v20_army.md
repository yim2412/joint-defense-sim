...
## 5b. DB 확충 체크리스트 (구현 시 공개 제원 대조 — [[project-db-realism]])

> **원칙: "많이"가 아니라 "빈 도메인 채우기".** 현 DB 실측(2026-07-12): ENEMY 69·SHIP 25·FRIENDLY 14·
> AIRCRAFT 9·AIR_FORCE 9·STRIKE 8·FLEET 아16/적28. **적 위협·함정·미사일은 이미 촘촘**(무작정 편성
> 규모화는 중복 시나리오만 늘어난다고 진단 소진). **진짜 빈 곳 = 아군 지상·연안 BMD.** 각 항목은
> 구현 시 공개 제원 대조 후 추가(과거 P-700·052C·071·J-10C 오류 정정 이력 — 추정 금지).

**🔴 v20 필수 (설계가 요구 — 없으면 지상 방공 불가):**
- [ ] **천궁-II (KM-SAM II)** — 연안 BMD 하층 종말 점방어(~40km). FRIENDLY_DB/어쇼어 자산. (v20.2a)
- [ ] **L-SAM** — 연안 BMD 상층(~150km·고고도, 한국형 THAAD급). 어쇼어 자산. (v20.2a)
- [ ] 패트리엇 PAC-3 — (선택) 한·미 연안 종말 방어.

**🟡 근미래 실존 플랫폼 (빈 곳·중복 낮음):**
- [ ] **KDDX (차기 구축함)** — SHIP_DB(현 KDX-III까지만). 실존 차세대.
- [ ] **KC-330 시그너스(공중급유기)** — AIR_FORCE_DB(현재 급유기 없음 → CAP 지속력 모델).
- [ ] 현무-II (지대지 탄도) — 지상 타격(v20 지상 도메인).
- [ ] 해병여단 등 상륙 부대 단위 — v20.3 상륙(함정 아닌 부대 개념, AmphibiousForce.troops).

**🟢 선택 (다양성·중복 위험 조금 — 여유 시):**
- [ ] 054B형(신형 적 호위함) — 현 054A만.
- [ ] 적 조기경보기(KJ-500 등) — 적 AEW 현재 없음.

**❌ 추가 금지(이미 있음/중복):** J-20·J-16·Su-35·Su-57·J-35(적 전투기 14종에 이미 포함) · 적 편성 규모 확대(ROI 소진 진단).

> **후속(post-v20, 사용자 2026-07-08 미룸)**: 공군 제공권 정교화용 대량 확충은 v20 완성 후 통합
> 상태에서 일괄(선택 트랙). 지금은 위 🔴🟡만 도메인 구현과 함께.
