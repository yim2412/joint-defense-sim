...
| 버전 | 범위 |
|------|------|
| **v14.1 (MVP)** | ✅ 완료 — 지구본 + 함정·항공기 위치 + 미사일 궤적 + 교전 이벤트 마커 + 타임라인 리플레이 (v14.01.01~06) |
| **v14.2 — 커버리지 돔** | ✅ 완료 (v14.02.01) — 레이더 탐지권 반투명 반구 돔. 아래 '커버리지 돔 설계' 참조 |
| v14.3 | 고도화 — 카메라 프리셋, 탐지권 돔 지형 차폐 비대칭 정밀화. (함정 종류별 아이콘은 사용자 결정으로 **철회** — 2026-06-12, stash·이미지 삭제) |

---

## ✅ 커버리지 돔 설계 (v14.02.01, 2026-06-12 완료)

**확정 방향**: 함대 대표로 **레이더 탐지권 1겹**만. (SAM 교전권 돔도 구현했으나 사용자 확인 결과 "교전권은 안 보이게" → 제거. 함정별 다중 표시도 밀집 시 지저분 → 단일화.)

| 돔 | 중심 함정 | 반경 | 색 |
|----|-----------|------|-----|
| 레이더 탐지권 | 최대 대공 센서 함정 | 함대 공유 실효 `detect_km` | 옅은 청록 (alpha 28) |

- **ellipsoid 반구 트릭**: 중심 고도 0(해수면) → 아래 반구는 지구에 가려져 위쪽 반투명 반구만 표시.
- 함정 `position` 시퀀스 재사용 → 돔이 함정 따라 이동, 격침 시 소멸(availability).
- `cesium_view.html` 좌상단 **"방어 커버리지 표시" 토글**(기본 ON, coverage 패킷 있을 때만 노출, `coverage/*` 엔티티 일괄 show).

**데이터 경로**: `_compile()` → `result['coverage'] = {'radar':{ship,km}}` (`_build_coverage()`, 순수 읽기·회귀 무영향). `build_czml()`의 `_dome()` 헬퍼가 ellipsoid 패킷 생성(헬퍼는 layer 인자라 향후 SAM 등 재추가 용이).

**미완(v14.3)**: 수평선·지형 차폐 비대칭(아래 설계). (함정 아이콘은 철회. SAM 교전권 돔이 필요해지면 `_build_coverage`에 sam 계산 + `_dome('sam', ...)` 복원 — git 히스토리 v14.02.01 직전 참조.)

---

## ✅ 차폐 정밀화 설계 (v14.03, 2026-06-12)

**목표**: 현 반구는 "고도↑일수록 좁아짐"으로 물리와 반대 → 반구는 **고고도 탐지권**으로 유지(옅게)하고, **저고도(해면) 탐지 경계를 한 겹 추가**(진하게). 두 형상 대비로 스탠드오프 가시화.

**표현 확정(사용자 2026-06-12)**: 저고도 경계 = **해면 탐지 외곽선** — 지표 클램프 닫힌 폴리곤(방위각 N=72 샘플). 육지 방위는 음영각만큼 안으로 패임.

| 단계 | 범위 | 데이터 |
|------|------|--------|
| **v14.03.01 수평선 차폐** | 저고도 경계 = 수평선 한계 원(대칭) | 순수 물리 `d≈4.12(√h_r+√h_t)`, 4/3 지구 |
| **v14.03.02 지형 차폐** | 그 경계를 육지 방위 섹터에서 음영각만큼 축소(비대칭) | `radar_shadow_reference`(태백 3.4°·설악 8.1°·소백 0.9°·낭림 0.4°) |

**데이터 경로**: `_build_coverage()`가 수평선·지형은 **엔진(물리)** 에서 계산 → `cov['radar']`에 `low_km`(수평선 한계)·`region` 추가. `build_czml()`은 방위각별 반경으로 폴리곤 좌표 생성(순수 형상). 엔진 교전 로직 불변 → 회귀 무영향.

**상수 가정**: 함정 레이더 안테나 고도 h_r≈25m, 해면 밀착 표적 h_t≈10m → 수평선 ≈ 33km. 저고도 경계 반경 = min(detect_km, horizon). 폴리곤 중심 = 레이더 함정 시작 위치 고정(교전 40초·저속이라 이동 무시 가능).
