...
def build_land_stock(ship_type: str, ship_db: dict) -> dict:
    """함정 1척의 지상공격 재고 — SHIP_DB default_inventory에서 지상공격 무기만 추출.
    없으면 빈 dict(대부분의 함정은 지상공격 능력이 없다)."""
    inv = (ship_db.get(ship_type) or {}).get('default_inventory', {}) or {}
    return {w: int(inv[w]) for w in LAND_ATTACK_WEAPONS if int(inv.get(w, 0)) > 0}
