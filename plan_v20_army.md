...
- 캠페인 교전 즉시예측·정밀 교전에서 해당 zone에 연안 SAM 있으면 **아군 방어 강도 가산**(win_p 보정).
- v18 EnemyADSite·C-RAM 고정 포대 로직 재사용. **이게 v20의 핵심 가치.**
...
if self.army is not None:
    ...
    # 교전 즉시예측·정밀 교전에 self.army.zone_defense_bonus(zone) 가산
```
...
## 8. 리스크 · 미결 (착수 시 사용자 합의 필요)

- **지형 기동 도입 여부**: 현 설계는 **연안 고정 포대 + 상륙 임무체인만**(기동 없음). 지상 기동전을
  원하면 별 프로젝트 — 이 범위에 넣지 않음(로드맵 권고).
- **ASBM 요격 모델**: DF-21D 대함탄도탄을 연안 SAM이 요격하는 물리(고고도 종말)를 어느 정밀도로?
  → 캠페인 즉시예측 층에선 **확률 가산**으로 추상화, 정밀 교전(A1) 경로에선 전술 엔진 SM-3/천궁 재사용.
- **상륙 실패의 outcome 가중**: 상륙 실패가 전역 패배에 얼마나 기여할지 = 기준값 측정 후 튜닝.
