...
- **✅ v18.4 전장의 안개**(v17.01.05): 교통로별 관측 기반 belief. `_tick_intel`(탐지원 3종: 초계 함정·초계기 ISR·위성) — belief는 **관측(도착분)만** 반영(soon 미래예측 제외 = 안개 핵심), 미탐지 zone 반감기 12h 감쇠. 임무 배정은 belief, 교전·SLOC은 truth. `enable_campaign_fog` 3종세트 기본 OFF(=완전정보 belief=truth, v18.3 bit-identical). 검증: fog ON에서 교전·생존 갈림, 동시 다발+자산부족 시 n_missed 발현, ISR 대수↑→과소평가↓(isr0 miss2→isr1 miss1). **설계 반복 교훈**: 초기엔 belief=truth(soon 포함)라 안개 무효 → 관측(now)만 반영으로 정정해야 발현.
- **▶ 다음 = v18.5 SLOC 정교화** (위협도 맵·우회로·보급 완충).
...
**✅ v18.5 구현 완료**(v17.01.06): 위 4종 + 부수 BUG 수정(`_all_ships_down`이 전원 일시 재보급을 전멸로 오판→조기종료 loss override가 mean_control 1.0인데 loss 만들던 모순 → '남은 horizon 내 복귀 불가'로 정교화·loss override는 surviving==0만). 이 수정으로 통제→보급 피드백 루프가 비로소 발현.

## 6-D. v18.6 캠페인 MC — ✅ 설계 확정 (2026-07-07, 사용자 승인)
**목표**: 캠페인 전역을 N회 반복해 outcome 분포·통제도·비용 통계 + **v18.5가 남긴 웨이브 낙관 편향(적 웨이브 3개 유한 + 무한 재교전 → 약편성 장기전 낙관)을 정량 확인**해 웨이브 스케줄 손질 여부 근거 확보.

**D1 트리거**: 캠페인 모드에서 기존 시뮬 모드 콤보(빠른/표준/정밀)를 **캠페인 전용 반복 횟수**로 매핑 — 전술 mc_n(5000/10000/100000)은 캠페인엔 과함(1회가 더 무거움). `[100, 300, 1000][sim_mode_idx]`, 테스트 모드 10. 캠페인은 이제 **항상 MC**(대표 단발은 seed 0 1회를 배너·시계열용으로 함께 실행).

**D2 실행 경로**: `engine_campaign.monte_carlo_campaign(cfg, n, model, progress_cb)` 신설 — `run_campaign`을 seed 0…N-1 스윕. **모델 1회 로드 공유**(3.1MB pkl 반복 로드 안 함). MVP **단일 프로세스 순차**(즉시예측이라 1000회 수초~2분 예상, 초과 시 후속 ProcessPool 병렬). **전술 MC 3경로(monte_carlo_v7·_mc_batch_worker·monte_carlo_lhs)와 완전 별개** — 캠페인 전용 집계 루프.

**D3 집계 지표**: outcome 분포(win/draw/loss 비율) + 평균/표준편차(mean_control) + 평균(잔존 함정·교전 수·누적 비용·재배정·우회·안개 놓침·종료 시각).

**D4 결과 표시(MVP)**: SimWorker 캠페인 분기에서 mc 결과를 대표 단발 result에 `campaign_mc` 키로 임베드 → `_render_campaign_result` 확장으로 배너·상태줄에 **outcome 분포 + 평균 지표** 텍스트 요약. **분포 히스토그램·시계열 시각화는 v18.7**(보고서 단계)로 분리.

**손댈 곳**: `engine_campaign.py`(`monte_carlo_campaign` 신설) · `app_main.py`(SimWorker 캠페인 분기 MC 라우팅 + `_render_campaign_result` MC 확장) · `_audit_campaign_smoke.py`(MC 경로 스모크). **검증**: 강편성 win 편중·약편성 loss/draw 편중, horizon별(72/168h) outcome 분포로 장기전 낙관 정량화. 전술 엔진 무수정→회귀 bit-identical·code-review high·GUI 스모크(캠페인 MC).
