...
2. **죽은 기능 게이트가 캠페인 층을 안 본다**(2026-07-16 v21.2서 발견) —
   `chk_effect_coverage`는 **`engine_combat.py`가 소비하는 플래그만** 커버를 요구한다.
   `enable_joint_fires`·`enable_air_campaign`·`enable_sead`·`enable_strategic_strike`·
   `enable_army_campaign`·`enable_coastal_sam`·`enable_amphibious`·`enable_enemy_sead`·
   `enable_precise_engagement`·`enable_campaign_fog` 등 **캠페인/공군/육군/합동 층 토글은
   전부 자동 면제** → 죽은 채 태어나도 커밋이 막히지 않는다. v20.5 부채(43개)를 만든 바로
   그 구멍이 **캠페인 층에는 아직 열려 있다.**
   - 부분 대응(v21.2): `audit_effect.CAMPAIGN_PROBES` 신설 — `run_campaign` ON/OFF 델타를
     재는 캠페인 프로브 틀 + 측정 유효성 자가검사(웨이브 0이면 즉시 실패). 신규
     `enable_joint_fires` 등재·확증(적출항 −0.195·통제 +0.044).
   - 남은 숙제: `consumed` 스캔을 engine_campaign·airforce·army·joint까지 확장. 켜는 순간
     기존 캠페인 토글 10여 개가 미커버로 FAIL하므로, **각각 캠페인 프로브를 만들거나
     EFFECT_ALIVE 등재**하는 별도 트랙이 필요하다(부채 청소 A~E와 같은 성격).
3. **단조성 불변식 미구현** — 보존식(요격≤총·rate=요격/총)은 property에 도입했으나
   "적↑→요격률↓" 단조성은 **노이즈(시드 분산)로 신뢰도 낮아 보류**. 평균 다수 시드
   필요 → 비용 대비 가치 낮음.
4. **골든 오라클 자기참조** — 회귀는 "이전과 같은가"만. 골든 갱신 시 "왜 바뀌었나"는
   사람 판단이 유일 오라클. property 스위트가 독립 오라클로 보강하나 완전 대체 불가
   (오라클 문제는 원리적으로 미해결 — 관리 대상).
5. **UI 시각적 깨짐·안 눌러본 경로 크래시(부분)** — 탭 렌더 크래시는 `_audit_render_smoke.py`
...
6. **GUI abort(중단) 실제 클릭 미자동화** — 전파는 헤드리스로 확인하나, MC 실행 중
   중단 버튼 클릭→워커 잔존 0 시나리오는 스모크에 없음(GUI 타이밍 난도).
7. **프리셋 편성 count ↔ 표기(changelog/주석) 정합** — 자유텍스트 파싱이라 자동화
   난도 높음(v16.05서 ARM 20 vs 표기 24 수동 발견). 미해소.
8. **비-enable_ 수치 키 전수 미점검 (부분 해소)** — 경계 5케이스는 property에 있고,
...
- **효과 검증(죽은 토글)**(v17.01.10) → `audit_effect.py` — 효과 입증 시나리오에서 ON/OFF
  ...
  잴 수 있게 `CAMPAIGN_PROBES` 추가**(단발 8종 + 캠페인 1종). 단 게이트가 캠페인 층을
  강제하진 않는다 → 열린 사각 2번.
