...
# F-017: 조기 수렴 종료 문턱(`conv_tol`). 요격률 누적 평균의 표준오차가 이 값 이하로
# 내려가면 남은 회차를 건너뛴다. 모드마다 다른 이유는 **필요한 정확도가 다르기 때문**이다 —
# 빠름은 '대충 어느 쪽이 나은가', 정밀은 '수치를 보고한다'. 0 이면 끄기(끝까지 돈다).
# 실측(이지스 vs 랴오닝, 200회 요청): tol 0.01 → 50회·66초(평균 0.4837)
#                                      tol 0     → 200회·251초(평균 0.4810) · 차이 0.0027
SIM_MODE_PRESETS = [
    ...
    # 정밀은 '수치를 보고하는' 모드라 문턱을 엄격히(±0.3%p). 사실상 끝까지 도는 경우가 많다.
