"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   합동 통합방어 시뮬레이터  v21.12.03 — PyQt6 런처                          ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  [v21.12.03 — 위협 추적표 격추거리 (D4)]                                    ║
║  BUG-1  격추거리가 요격 56건 중 48건 빈칸 — 요격탄↔표적 근접신관           ║
║         간격(≤0.2km)을 적고 있었다. 방어 함정↔격추 지점 거리로.            ║
║  BUG-2  0.0km(함정 직상공 CIWS 격추)가 빈칸으로 지워지던 문제.               ║
║  BUG-3  기만기·회피로 막은 어뢰의 격추 수단이 빈칸이던 문제.                 ║
║  [v21.10.01~02 — 자폭형 위협 집계 정의 정리]                                ║
║  NEW-A  위협 무력화율 신설 — 자폭 드론·자폭정은 미사일을 쏘지 않아          ║
║         요격률 분모·분자 어디에도 안 잡혔다. 격퇴해도 방어 실적에          ║
║         반영되지 않아 요격률이 아군을 **과소평가**했다(무인기 군집          ║
║         포화 실측 50% → 96.2%). 요격률은 값·정의 그대로 유지.              ║
║  BUG-1  자폭에 성공한 수상 자폭정이 '적 격침'에 집계돼 적의 성공이          ║
║         아군 전과로 잡혔다. 자폭 성공 표시를 분리해 차단.                   ║
║  [v21.09.03 — 시나리오 규모 현실화 (프리셋 전수 측정 후속)]                 ║
║  BUG-1  '북한 탄도 포화'·'러시아 극초음속'이 이름과 달리 각 6발이라         ║
║         어떤 편성으로도 요격률 100%·손실 0이었다. 40발·38발로 확대 —        ║
║         단독함 생존 0% ↔ 전 이지스 98%로 편성 변별력이 생겼다.              ║
║  [v21.09.01~02 — 3D 전장 표현 (사용자 직접 지적분)]                         ║
║  BUG-1  요격 성공과 아군 피격이 같은 노란 점이라 구분 불가였다.             ║
║         하늘색 '✔ 요격 · 무기명' / 붉은색 '💥 피격 · 위협명'으로 분리.      ║
║  BUG-2  아군 미사일 종말 마커가 `if is_enemy:`로 통째로 빠져 있었다.        ║
║  BUG-3  미사일에 이름표가 없어 클릭해야 무엇인지 알 수 있었다.              ║
║  BUG-4  범례가 없어 색의 의미를 알 수 없었다. 명중 표시 2.5초→6초.          ║
║  [v21.08.08 — 요구조건 기준을 분석자 입력으로 (묶음 B)]                     ║
║  NEW-A  REQ 5항목 임계값을 cfg로 지정 가능(미지정 시 기존 값 그대로).       ║
║         항목 이름도 실제 기준을 표시한다. 고정 임계값은 모델이 바뀔         ║
║         때마다 변별력을 잃는다 — 7조합 실측에서 REQ-04·05는 전부 실패,      ║
║         REQ-08은 전부 통과였고, 반대로 REQ-02·07은 되살아났다.              ║
║  [v21.08.06~07 — 편성 비교가 성립하는 지표 (묶음 B)]                        ║
║  NEW-A  방어 포화도(최대 동시 위협 ÷ 총 교전 채널)를 1면에 병기. 분모가     ║
║         편대 자신이라 규모가 달라도 비교된다 — 요격률이 못 하는 일이다.     ║
║         MC 3경로에 총위협·최대동시·채널·포화도 집계를 함께 추가.            ║
║  BUG-1  등급 배너가 상황과 무관하게 '함정 수 보강'을 처방하던 문제.         ║
║         채널 초과 구간에서는 함정을 늘려도 요격률이 오르지 않는다.          ║
║         포화도를 함께 보고 CEC·분산 또는 무기 구성 조정으로 갈라 안내.      ║
║  [v21.08.03~05 — 기본 설정을 현재 모델로 (묶음 A-3)]                        ║
║  BUG-1  비례항법(PNG) 기본 적용 승격. 실제 미사일이 쓰는 유도 법칙이라      ║
║         선택 옵션이 아니다. 격추 거리 표시가 25%→100%로 살아난다.           ║
║  BUG-2  저장본을 열면 환경물리 5종(지형·덕팅·ISA·소나·침수)이 꺼진 채       ║
║         복원되던 문제 — 같은 설정이 경로에 따라 다른 물리로 돌았다.         ║
║  BUG-3  '실험적' 26개 중 8개 정규 승격(효과 실측 확인분). 레이저·능동       ║
║         소나는 한계를 도움말에 명시하고 표기 유지. BMD 5계층은 공개         ║
║         문서에서 '선택 옵션'으로 표기 정정.                                 ║
║  [v21.08.01~02 — 편성 비교가 성립하는 무대를 만든다 (묶음 A-1·A-2)]         ║
║  BUG-1  적 항모가 함재기 전량(24기)을 대함 타격에 투입해, 탑재 104발짜리    ║
║         전단이 190발을 쐈다. 초과 80발이 전부 함재기분. 대함 투입 비율을    ║
║         분리(24→8기) — 보유 대수는 공개 제원 그대로 둔다.                   ║
║         랴오닝 대역 요격률 11.4%→84.8%(7척)·전 편성 전멸 해소.              ║
║  BUG-2  '입체 포화 (최강)'이 37발로 랴오닝(190발)보다 5배 약했다.           ║
║         정밀 위협 1발 편성이 원인 — 다축 동시 포화로 확대(107발).           ║
║         7척 요격률 100%→88.1%로 실제 최고 난이도가 됐다.                    ║
║  ※ 회귀 골든 갱신 1회(8케이스 중 4케이스 120건 — 두 프리셋 사용분만).       ║
║  [v21.07.15~17 — 위생 정리 (묶음 C-5)]                                      ║
║  BUG-1  읽지 않는 지상 위협 자료가 실행 파일에 번들되던 문제(자료는 보관).  ║
║  BUG-2  시작 화면에 같은 엠블럼이 두 번 나오던 문제.                        ║
║  BUG-3  향후 계획 탭에 완료 항목·개발 절차 규칙이 남아 있던 문제 — 현재     ║
║         로드맵(지표 교체·기본값 현대화·3D·전력 DB·대잠전)으로 재작성.       ║
║  [v21.07.10~14 — 첫 화면을 읽을 수 있게 (묶음 C-3·C-5 일부)]                ║
║  BUG-1  9~11px 설명 글씨 38곳을 12px로. 가장 작은 글씨가 판단 근거였다.     ║
║  BUG-2  미체크 라벨 빨강→회색. 54개 중 36개가 기본 OFF라 화면의 2/3가       ║
║         경고색이었다. 녹색 채움+빨간 글씨는 적록 색각에 최악 조합.          ║
║  BUG-3  글자 잘림 — 스크롤바 자리 확보 + 버튼 높이 상향 + 긴 라벨 축약.     ║
║  BUG-4  정밀도 안내 문구가 실제 횟수와 달랐다(문구가 숫자를 따로 보유).     ║
║  DEL-A  발동 경로가 없는 '적 Anti-SAM 방어' 옵션을 화면에서 제거.           ║
║  [v21.07.09 — 정밀도 모드 반복 횟수 현실화 (묶음 C-4)]                      ║
║  BUG-1  표준 1회 실행이 약 1.1시간이라 편성을 바꿔가며 비교할 수 없었다.    ║
║         실측 표준편차 4.67%p 기준 1,000회면 평균 오차 ±0.15%p로 수렴한다    ║
║         (10,000회는 오차 0.1%p를 더 줄이려고 시간을 10배 썼다). 약 9분으로. ║
║         빠름 5,000→200 · 정밀 100,000→10,000 · 민감도 표본 98,304→12,288.   ║
║  [v21.07.06~08 — 엔진 값이 화면까지 도달하게 (묶음 C-2)]                    ║
║  BUG-1  위협 추적표 '유형'이 전부 '?' 였던 문제. 플랫폼이                   ║
║         발사한 미사일·어뢰에 위협 유형을 부여했다.                          ║
║  BUG-2  '탐지거리'가 전부 0 이던 문제. 수색 레이더가 최초로                 ║
║         포착한 순간의 거리를 기록한다(어뢰는 '—').                          ║
║  BUG-3  항공 출격 카드가 '0회'와 '미편성'을 같은 '—'로 표시하던 문제.       ║
║  [v21.07.01~05 — 표시 오류 5건 정정 (묶음 C-1)]                             ║
║  BUG-1  설정 화면 교전통제 고정 시간이 'C&&D'로 표시되던 문제.              ║
║  BUG-2  승리가 없는 편성의 예상 전과가 '승리당 승리 없음'으로 깨지던 문제.  ║
║  BUG-3  분석 차트·Excel 보고서 표제가 옛 이름(v7.0)으로 남아 있던 문제.     ║
║  BUG-4  시작 화면 부제가 해군 단독 범위로 남아 있던 문제.                   ║
║  BUG-5  전술 요구조건 수가 실제(7가지)와 다르게 8가지로 표기되던 문제.      ║
║  [v21.06.05 — 창 위치 복원이 실제로 동작하게 정정]                          ║
║  BUG-1  보조 모니터에 창을 두고 닫아도 다음 실행 때 주 모니터 중앙으로      ║
║         돌아오던 문제. 창 위치를 복원한 뒤 "화면 밖인지" 검사할 때          ║
║         주 모니터만 기준으로 삼아, 보조 모니터의 창을 화면 밖으로          ║
║         오인해 끌어오고 있었다. 연결된 모든 모니터를 기준으로 판정하게     ║
║         정정 — 모니터를 실제로 제거한 경우의 보정은 그대로 유지된다.       ║
║  [v21.06.04 — 적정 편대 추천 기준 교체 (생존·교환비 중심)]                  ║
║  BUG-1  편대 추천이 규모가 작은 편대를 더 높게 평가하던 문제.               ║
║         요격률이 편대 규모에 따라 비교 기준이 달라져(같은 시험이 아님)      ║
║         구축함 1척이 이지스 8척 편대보다 높게 나왔다. 기준을 생존율·        ║
║         교환비 중심으로 바꿔 최강 방공 편대가 1위, 단독 구축함이            ║
║         최하위로 정정됐다. 지속 전장 모드 기준(승률·임무점수)은 유지.       ║
║  [v21.06.03 — 창 위치를 기억해 같은 자리에서 열린다]                        ║
║  BUG-1  창 위치·크기를 저장만 하고 복원하지 않아, 껐다 켤 때마다           ║
║         기본 위치에서 열리던 문제. 모니터를 여러 대 쓰면 켤 때마다         ║
║         창을 옮겨야 했다. 이제 닫을 때의 화면·위치에서 그대로 열린다.      ║
║         (홈 화면·본 창 모두. 화면 구성이 바뀌면 자동 보정)                 ║
║  [v21.06.02 — 항모 항공단 한계 정규 승격 (기본 ON)]                         ║
║  NEW-A  항모 항공단 한계를 정규 기능으로 승격하고 기본 활성화.             ║
║         탑재량은 항상 존재하는 물리이고, 끈 상태가 오히려 비현실           ║
║         (적 항공 전력 무제한)이었다. 기준 시나리오에서 어떤 편성을         ║
║         골라도 전멸하던 것이 편성에 따라 갈리게 된다.                      ║
║         (한미 기동전단 강화 전멸률 100% → 35%)                             ║
║  [v21.06.01 — 항모 항공단 한계 옵션 신설 (실험적·기본 OFF)]                 ║
║  NEW-A  적 항모가 실제 탑재 항공단 규모만큼만 함재기를 발진하는 옵션.       ║
║         (랴오닝 24기·산둥 36기·푸젠 40기) 지금까지 단발 교전에서는 항모가   ║
║         교전 내내 함재기를 계속 발진시켜 적 항공 전력이 사실상 무제한이었다.║
║         실측: 랴오닝 항모전단 상대 위협 438→211발·요격률 44.3→60.7%,       ║
║         전멸률 97.5→30.0%. 지속 전장 모드에는 이미 적용돼 있던 규칙을       ║
║         단발 교전에도 선택 적용할 수 있게 한 것(기본 OFF = 기존 결과 동일). ║
║  [v21.05.01 — 설치 용량 39% 감축(1.2GB→734MB) + 번들 과잉 자동 점검]        ║
║  NEW-A  빌드에 포함되던 미사용 대형 라이브러리 제거 — 앱이 쓰지 않는        ║
║         딥러닝 런타임 등 445MB가 실려 다니고 있었다(학습 도구 전용).        ║
║         AI 전술 추론은 경량 수치 연산만 쓰므로 기능 영향 없음.              ║
║         빌드 시간도 384초 → 170초로 단축.                                   ║
║  NEW-B  번들 과잉 자동 점검 신설 — 감사가 "빠진 게 없는가"만 보고            ║
║         "불필요한 것이 들어갔는가"는 보지 않던 사각을 메운다.               ║
║  NEW-C  화면 점검 자동화를 보조 모니터에서 수행(주 화면 점유 방지).         ║
║  [v21.04.02 — 잠수함 현무-3C 대함 비물리 정정 + 합동 화력 연결]             ║
║  BUG-1  KSS-III 잠수함이 지상공격 순항미사일 현무-3C로 적 수상함을 치던     ║
║         비물리 제거(_select_sub_strike_wpn에서 현무-3C 제외) — SM-3         ║
║         외기권↔대기권·SAM 대공↔어뢰와 같은 '잘못된 무기가 잘못된 표적' 계열.║
║         잠수함 대함은 하푼(KSS-I)·어뢰로 한정.                              ║
║  NEW-A  KSS-III 현무-3C를 올바른 소비처(합동 화력 지상공격)로 연결 —        ║
║         build_land_stock이 default_strike_inventory도 읽어 잠수함 발사      ║
║         현무-3C가 적 항구·비행장 타격에 참여(수상함은 이중계상 없음).       ║
║         검증: KSS-III 3척 = 합동 화력 순항 18발 기여 · 회귀 bit-identical    ║
║         (골든에 KSS-III 없음).                                              ║
║  [v21.04.01 — 합동 작전 시나리오 라이브러리 3종 (v21.3)]                    ║
║  · 육해공 통합 캠페인 시나리오 3종(전면전·대만해협·제한전) 원클릭 프리셋.   ║
║                                                                              ║
