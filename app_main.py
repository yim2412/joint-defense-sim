...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            # ── v17.x — 군수·미래 전장 ────────────────────────────────────────
            ("v17.2", "낮음", "지향성 에너지 무기 (레이저) — 실효 없음(v20.5에서 최종 판정)",
             ...
             "캠페인에서 재평가한다. "
             "【v20.5 관점】함대공 미사일이 저속 드론을 원거리에서 먼저 잡아 레이저 차례가 오지 않는다는 "
             "점은 SM-3 사례와 겉모습이 같으나, 결정적으로 다르다 — SM-3는 물리적으로 불가능한 요격을 "
             "했지만(오류), 함대공이 드론을 20km에서 잡는 것은 물리적으로 가능하다(오류 아님). "
             "현실의 함정 레이저도 아직 드론 방어의 주력이 아니므로, 이 음성 결과 자체가 현실 반영일 수 "
             "있다 — v20.5에서 최종 판정 후 로드맵에서 종결한다."),
            ...
            ("v19.6", "높음", "다전장 연동 (공군·지상 합동 교전구역) — 선행 조건 충족, 착수 가능",
             "공군 방공망 제압·지상 패트리엇/THAAD를 해군 캠페인 엔진과 연동. 합동 교전규칙 구역 자동 분할. "
             "선행 조건이던 지상 방공망(v20.2)이 완성되어 이제 착수 가능하다. "
             "v20.5 유효성 재점검을 마친 뒤 진행하며, 합동작전(v21)의 사실상 전 단계에 해당한다."),
            # ── v20.x — 육군 작전급 ────────────────────────────────────────────
            # 선행 필수: v18 완성
            # ── v21.x — 육해공 통합 합동작전 ──────────────────────────────────
