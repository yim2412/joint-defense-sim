...
class SplashWindow(QWidget):
    ...
    def _build_plan_tab(self) -> QWidget:
        ...
        _PLANS = [
            ...
            ("v18.2", "높음", "전력 관리 모델",
             "전 함정 상태 추적: 초계·교전·귀항·수리·재보급. "
             "손상 함정 수리 기간(피해 심각도별 1~14일), 탄약 재보급 시간. "
             "보급함 이동·미사일 재장전·연료 소모 등 군수·보급 모델 포함. "
             "가용 전력 = 전체 − 수리중 − 재보급중. "
             "【구현 완료】피해 심각도별 수리 기간 차등, 교전당 탄약 소모·초계 연료 소모, "
             "소진 시 모항 재보급 귀항. 결과에 수리·재보급 함정 수·평균 탄약/연료 표시. "
             "다음: 임무 배정(가용 전력 → 초계·호위·타격 우선순위)."),
            ("v18.3", "높음", "임무 배정 시스템",
             "가용 전력을 여러 임무에 자동 배정: 초계·호위·타격·대잠·보급 호위. "
             "우선순위 기반 배정(이지스급 → 고위협 구역 우선), 임무 충돌 해결. "
             "【구현 완료】위협 기반 동적 재배정(6시간 주기) — 방공 능력이 강한 함정을 고위협 "
             "교통로에 방어집중, 위협 없는 교통로엔 초계 함정 유지. 교통로 간 이동 시간 소요. "
             "다음: 정보 불확실성(전장의 안개) → SLOC 정교화 → 캠페인 반복 분석."),
