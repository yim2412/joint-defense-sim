...
SHIP_DB = {
    ...
    'KDX-III-B1': {
        ...
        'default_inventory': {
            ...
            '홍상어 (대잠)':     16,   # KVLS-I
            # v21.2: KVLS-I 48셀 = 현무-3C 32 + 홍상어 16(공개 보도값). 홍상어 16이 이미
            # 있어 정확히 맞물린다. 현무-3C는 지상공격 순항미사일이라 대함 무기 선택
            # (_select_strike_wpn)에 없다 → 단발 교전에선 불활성(골든 무변), 작전급
            # 합동 화력(engine_joint)이 적 기지 타격에 소비한다.
            '현무-3C':           32,   # KVLS-I
            '청상어 (경어뢰)':   12,
            'Mk.46 경어뢰':       8,
    ...
    'KDX-III-B2': {
        ...
        'default_inventory': {
            ...
            '홍상어 (대잠)':      8,   # KVLS-I
            # v21.2: KVLS-II 24셀이 현무 탄도/순항 자리(위 주석). 순항형만 모델링해
            # 현무-3C 16 — 나머지 8셀은 탄도형(현무-2 계열) 몫으로 남긴다(미모델).
            # B1과 같이 대함 선택 경로엔 없어 단발 교전 불활성.
            '현무-3C':           16,   # KVLS-II
            '청상어 (경어뢰)':   12,   # 어뢰발사관
            'Mk.46 경어뢰':       8,   # 어뢰발사관
            'CIWS-II (Phalanx)': 9999,
    ...
    #   KDX-III보다 작고 채널 수도 적지만, 스텔스·전기추진으로 피탐·소음이 낮다.
    #   함대공은 국산 해궁(단거리)을 주력으로 하되, 한미 상호운용 SM 계열을 보수적 소량 편성한다
    #   (KDDX 전용 국산 장거리 함대공은 아직 실체가 공개되지 않아 기존 무기 DB로 근사).
    'KDDX': {
        'display':      '차기 구축함 (KDDX)',
        ...
        'role':         ['대공', '대함', '대잠', 'BMD'],
        # VLS 64셀 = KVLS-I 48 + KVLS-II 16. 총 탑재 SAM은 셀 수를 넘지 않는다(8+16+24=48 ≤ 64).
        'default_inventory': {
...
FLEET_PRESETS = {
    ...
    # v20.1: 차기 기동전단 (KDDX 전력화 이후 — 이지스1 + KDDX2). 통합 전기추진·통합 마스트로
    #   피탐·소음이 낮고, 국산 다기능 레이더가 탄도탄을 탐지·추적한다.
    '차기 기동전단 (KDDX)': [
        {'name': '정조대왕함', 'type': 'KDX-III-B2'},
        {'name': 'KDDX-1',     'type': 'KDDX'},
        {'name': 'KDDX-2',     'type': 'KDDX'},
    ],
...
ENEMY_FLEET_PRESETS = {
    ...
    # BMD 탄도 포화 — 순수 탄도·HGV 방어 전용 (BMD 다층 요격 성능 평가)
    # HGV 4발(과거 1발)로 상향: 실제 교리의 탄도 공격은 다축 대량 동시 포화이고,
    # 1발 편성은 상층 SM-3가 단독으로 전부 처리해 하위 4계층(THAAD·L-SAM·패트리엇·
    # 천궁-II)이 발사조차 못 했다 — '포화'라는 이름과 달리 다층 요격을 평가할 수 없는 규모.
    'BMD 탄도 포화': [
        {'preset': 'KN-23 (북한 이스칸데르)',   'count': 6},   # QBM: SM-3 무력화, SM-6/ESSM 대응
        {'preset': 'DF-15 (단거리 탄도)',       'count': 6},   # SRBM: SM-3 주 요격
        {'preset': 'DF-21D (대함 탄도)',        'count': 4},   # MRBM: SM-3 필수
        {'preset': 'DF-17 (극초음속 활공)',     'count': 4},   # HGV: SM-3만 가능, 최고 난이도
    ],
    ...
    # 북한 탄도 포화 — 화성 계열 + 순항
    # ⚠ 2026-09-19 현실화: '포화'라는 이름인데 총 6발이라 이지스 6척 상대로 요격률 100%·
    #   손실 0이었다(docs/analysis/04). 북한은 이동식 발사대를 다수 운용해 KN-23을
    #   한 번에 수십 발 쏠 수 있고, 실제 교리도 그 대량 동시 발사가 핵심이다.
    '북한 탄도 포화': [
        {'preset': 'KN-23 (북한 이스칸데르)',      'count': 24},
        {'preset': '화성-15 (북한 ICBM급)',        'count': 4},
        {'preset': '북한 순항미사일 (화살-2)',      'count': 12},
    ],
    # 러시아 극초음속 — 킨잘·지르콘·스텔스 순항
    # ⚠ 2026-09-19 현실화: 총 6발이라 요격률 100%였다. 킨잘은 탑재기(MiG-31K)가 제한적이라
    #   10발 안팎이 상한에 가깝고, 지르콘·Kh-101은 함상·폭격기 발사라 더 많이 나온다.
    '러시아 극초음속': [
        {'preset': '킨잘 (극초음속 탄도)',    'count': 10},
        {'preset': '지르콘 (극초음속 순항)', 'count': 12},
        {'preset': 'Kh-101 (스텔스 순항)', 'count': 16},
    ],
    ...
    # v20.1: 차세대 수상 전투단 — 054B형(2025 취역) 중심. 054A 대비 자체방어·근접방어가
    #   강화돼(Type 1130 CIWS + HHQ-10) 아군 대함미사일이 더 많이 요격당한다.
    '차세대 수상 전투단 (054B)': [
        {'preset': '055형 대형 구축함', 'count': 1},
        {'preset': '052D형 구축함',     'count': 2},
        {'preset': '054B형 호위함',     'count': 3},
        {'preset': '054A형 호위함',     'count': 1},
    ],
    ...
    # ⚠ 2026-09-19 현실화: 이전 편성(J-20×2·DF-17×1·055×1·093×1)은 실제 발생 위협이
    #   37발뿐이라 '최강'인데도 랴오닝 항모전단(190발)보다 5배 약했다. 7척 편성 상대로
    #   요격률 100%·손실 0이 나와 최고 난이도 라벨과 정반대였다. 원인은 **정밀 위협을
    #   1발씩만 편성**한 것 — CLAUDE.md 감사 규칙 ②가 경고하는 패턴이고, 실제 교리는
    #   다축 **대량 동시 포화**다. 각 축을 편대 규모로 올려 라벨과 실제를 맞춘다.
    '입체 포화 (최강)': [
        {'preset': 'J-20 (위룡)',              'count': 4},  # 공중: 스텔스 전투기 2개 편대
        {'preset': 'DF-17 (극초음속 활공)',    'count': 4},  # 공중: HGV 포화 (SM-3 필수)
        {'preset': '055형 대형 구축함',        'count': 2},  # 수상: VLS 112셀 YJ-21
        {'preset': '052D형 구축함',            'count': 2},  # 수상: 호위 구축함
        {'preset': '093형 잠수함 (상급)',      'count': 2},  # 수중: 핵잠 어뢰·잠대함
    ],
