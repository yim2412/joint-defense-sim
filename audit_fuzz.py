...
경계값 버그가 별도로 잠재할 수 있다. (캠페인은 수치 cfg.get 키가 없어 pairwise가 커버.)

사용:
    python audit_fuzz.py              # 단발 fuzzing (발견 있으면 exit 1)
    python audit_fuzz.py --mode battle   # 전장 fuzzing
    python audit_fuzz.py --mode all      # 단발+전장 둘 다
"""
...
def _bases():
    ...
    return {
        'single': ('단발', run_v7_simulation, single_base),
        'battle': ('전장', run_battle_simulation, battle_base),
    }
...
def main():
    ...
    modes = ['single', 'battle'] if mode == 'all' else [mode]

    keys = extract_numeric_keys()
    n_inject = len(keys) * len(EXTREMES)
    print(f"수치 cfg 키 {len(keys)}개 × 극단값 {len(EXTREMES)}종 ≈ {n_inject}회/경로 fuzzing")

    bases = _bases()
    total = []
    for m in modes:
        if m not in bases:
            print(f"❌ 알 수 없는 모드: {m} (single|battle|all)")
            return 2
        label, run_fn, base = bases[m]
