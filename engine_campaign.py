...
class CampaignEngine:
    ...
    def run(self) -> dict:
        for self.t_h in range(1, self.horizon_h + 1):
            ...
            self._assign_missions()     # v18.3: 위협 기반 동적 재배정(6h마다)
    ...
    def _zone_threat(self, zone: str) -> float:
        """zone 위협도 = 이미 도달한 웨이브 규모 + 임박(다음 재배정 주기 내 도착) 규모×0.5.
        임박분을 반영해 웨이브 도착 전에 선제 배치할 수 있게 한다."""
        now = soon = 0.0
        for w in self.waves:
    ...
    def _return_zone(self) -> str:
        """수리·재보급 완료 함정의 복귀 교통로 — 결정론적 최고위협 zone(위협 없으면 첫 교통로)."""
        threats = {z: self._zone_threat(z) for z in SLOC_ZONES}
        if max(threats.values(), default=0.0) > 0:
            return max(SLOC_ZONES, key=lambda z: threats[z])
        return SLOC_ZONES[0]
