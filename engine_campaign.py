...
# v21.1 JCS 충돌 경고 임계 — 관찰 only(교전 무영향). 지휘부가 '경고로 알릴 만큼'의 문턱.
#   보수적으로 잡고 실측으로 조정한다(공개 교리에 정밀 문턱값 없음 → 근거 없는 확률 대신 카운트).
_JCS_OVERKILL_INFO     = 10      # 완파 표적 화력 중복(넘김) 누적이 이 이상이면 info 경고
...
class CampaignEngine:
    ...
    def _compile(self) -> dict:
        ...
        # v21.1 JCS 충돌 경고 — 상시 관찰(토글 없음). 위에서 병합된 각 층 지표를 지휘부
        # 관점에서 읽어 충돌을 명명한다. ⚠ result의 교전·배정 값은 절대 바꾸지 않는다.
        warns = self._jcs_warnings(result)
        result['jcs_warnings']      = warns
        result['jcs_warning_count'] = len(warns)
        return result

    # ── v21.1 JCS 충돌 경고 ─────────────────────────────────────────────────
    def _jcs_warnings(self, result: dict) -> list:
        """각 군이 독립 배정한 결과의 충돌을 지휘부 관점에서 경고로 표면화.

        ⚠ **관찰 only** — 교전 결과·자산 배정을 바꾸지 않는다(경고 리스트만 생성).
        _PLANS 사양: "완전 자동최적화는 현실에 없음(각 군 자율성) → 조정·충돌경고 수준이
        현실적". 실질 조정(overkill 시 잔여 표적 화력 넘김·CAP 상한 유지)은 이미 하위
        층(JointFires·공군 _assign)이 수행하고, 여기선 그 결과를 **알리기만** 한다.

        반환: [{'type','severity'('info'|'warn'|'crit'),'value','msg'}] 심각도 순.
        이미 병합된 result의 공개 지표만 읽어 층 내부에 손대지 않는다(디커플링)."""
        warns = []
        # ① 협조 미비 — 동시 타격 모드에서 화력지원 협조수단(FSCM) 없이 유인 폭격 소티 취소
        n_dec = int(result.get('joint_deconflict', 0))
        if n_dec > 0:
            warns.append({'type': 'deconflict', 'severity': 'warn', 'value': n_dec,
                          'msg': f"동시 타격 협조 미비 — 유인 폭격 소티 {n_dec}건 취소(FSCM 미설정)"})
        # ② 육군 미참여 — 지대지 화력을 편성했으나 지상 작전급 미가동이라 한 발도 못 씀
        if bool(result.get('joint_army_unused', False)):
            warns.append({'type': 'army_unused', 'severity': 'warn', 'value': 1,
                          'msg': "육군 지대지 화력 편성됐으나 지상 작전급 미가동 — 합동 화력 미참여"})
        # ③ 화력 중복 — 완파 표적에 화력이 반복 배분돼 잔여 표적으로 넘어간 누적(낭비 규모)
        n_ovk = int(result.get('joint_overkill_skip', 0))
        if n_ovk >= _JCS_OVERKILL_INFO:
            warns.append({'type': 'overkill', 'severity': 'info', 'value': n_ovk,
                          'msg': f"완파 표적 화력 중복 {n_ovk}회 — 지휘부가 잔여 표적으로 재분배(낭비 방지)"})
        # 심각도 순 정렬(crit→warn→info)로 지휘부가 위중한 것부터 본다
        _rank = {'crit': 0, 'warn': 1, 'info': 2}
        warns.sort(key=lambda w: _rank.get(w['severity'], 9))
        return warns
...
_MC_ACC_KEYS = ('mean_control', 'surviving_ships', 'n_engagements', 'cost_total',
                ...
                # v21.1 JCS: 충돌 경고 수(스칼라). jcs_warnings(list)는 위 dict/list와 같은
                #   사유로 제외 — 반복 분석엔 평균 경고 수만 남긴다.
                'jcs_warning_count')
...
def monte_carlo_campaign(cfg: dict, n: int, model=None, progress_cb=None) -> dict:
    ...
    return {
        ...
        # v21.1 JCS: 반복 캠페인 평균 충돌 경고 수(집계는 _MC_ACC_KEYS, 노출은 여기)
        'jcs_warning_count_avg':   round(acc['jcs_warning_count'] * inv, 2),
