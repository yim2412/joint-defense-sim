...
| v14.3 | 고도화 — 3D 함정 모델, 카메라 프리셋, 지형 차폐 정밀화, **비주얼·그림체 다듬기**(점→함정 아이콘, 색감·마커·라벨 스타일) ← 사용자 요청(2026-06-11), 기능 안정화 후 |
...
**미완(v14.3+)**: 수평선·지형 차폐 비대칭, 함정 종류별 아이콘. (SAM 교전권 돔이 필요해지면 `_build_coverage`에 sam 계산 + `_dome('sam', ...)` 복원 — git 히스토리 v14.02.01 직전 참조.)
